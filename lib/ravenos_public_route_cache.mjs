...
function remember(key, response, expiresAt) {
  memoryResponses.set(key.url, { response: response.clone(), expiresAt });
  while (memoryResponses.size > PUBLIC_ROUTE_RESPONSE_CACHE_POLICY.maximum_entries_per_isolate) {
    memoryResponses.delete(memoryResponses.keys().next().value);
  }
}
...
export async function readPublicRouteResponseCache({
  ...
  const memory = memoryResponses.get(descriptor.key.url);
  if (memory && memory.expiresAt >= nowMs) {
    return restoredResponse(memory.response.clone(), "isolate_hit");
  }
  if (memory) memoryResponses.delete(descriptor.key.url);
  ...
  try {
    ...
    remember(descriptor.key, response, storedAt + descriptor.policy.ttl_seconds * 1_000);
...
export function storePublicRouteResponseCache({
  ...
  remember(descriptor.key, stored, nowMs + descriptor.policy.ttl_seconds * 1_000);
