...
export const DISCOVER_REVIVAL_SCAN_SCHEMA = "ravenos.discover_revival_scan.v2";
export const DISCOVER_CLASSIFIER_NAME = "raven_behavioral_radar";
export const DISCOVER_CLASSIFIER_VERSION = "2026-09-03.1";
export const DISCOVER_MARKET_FACT_TARGET_SECONDS = 120;
export const DISCOVER_REVIVAL_MIN_AGE_SECONDS = 30 * 86_400;
export const DISCOVER_REVIVAL_MIN_QUIET_SECONDS = (24 * 60 - 5) * 60;
...
function revivalScanEvidence(row, observedAt, freshnessState = "current") {
  ...
  const currentFlow = flow(row, "5m");
  const baselineFlow = flow(row, "1h");
  const dayFlow = flow(row, "24h");
  const currentVolume = metric(row, "volume_usd", "5m");
  const baselineVolume = metric(row, "volume_usd", "1h");
  const priceChangePct = metric(row, "price_change", "5m");
  const transactionRateAcceleration = rateAcceleration(currentFlow.transactions, 5, baselineFlow.transactions, 60);
  const participantRateAcceleration = rateAcceleration(currentFlow.participants, 5, baselineFlow.participants, 60);
  const volumeRateAcceleration = rateAcceleration(currentVolume, 5, baselineVolume, 60);
  ...
  const rollingCountsConsistent = rollingCountsAvailable
    && dayFlow.transactions >= baselineFlow.transactions
    && baselineFlow.transactions >= currentFlow.transactions;
  const precedingHourTransactions = rollingCountsConsistent
    ? baselineFlow.transactions - currentFlow.transactions
    : null;
  const precedingDayTransactions = rollingCountsConsistent
    ? dayFlow.transactions - currentFlow.transactions
    : null;
  const dormancyProven = rollingCountsConsistent
    && precedingHourTransactions === 0
    && precedingDayTransactions === 0;
  const flowSignals = [
    transactionRateAcceleration !== null && transactionRateAcceleration >= 0.25,
    participantRateAcceleration !== null && participantRateAcceleration >= 0.2,
    volumeRateAcceleration !== null && volumeRateAcceleration >= 0.25,
  ].filter(Boolean).length;
  const positiveOrientation = (priceChangePct !== null && priceChangePct > 0)
    || (currentFlow.buy_share !== null && currentFlow.buy_share >= 0.52);
  const priceSignal = priceChangePct !== null && priceChangePct >= 1.5;
  ...
  else if (!rollingCountsAvailable) reasonCode = "dormancy_history_unavailable";
  else if (!rollingCountsConsistent) reasonCode = "rolling_counts_inconsistent";
  else if (!dormancyProven) reasonCode = "prior_activity_present";
  else if (flowSignals < 1 || (flowSignals < 2 && !priceSignal)) reasonCode = "rate_burst_unconfirmed";
  else if (!positiveOrientation) reasonCode = "positive_reclaim_unconfirmed";
  return Object.freeze({
    ...
    preceding_1h_transactions: precedingHourTransactions,
    preceding_24h_transactions: precedingDayTransactions,
    ...
    source_scope: "exact_pool_current_and_rolling_transaction_windows",
    observed_at: iso(observedAt),
    freshness: freshnessState,
  ...
  for (const row of value.rows) {
    ...
    if (
      ...
      || (discovery.revival_scan?.qualified === true && (
        ...
        || discovery.revival_scan?.dormancy_proven !== true
        || discovery.revival_scan?.preceding_1h_transactions !== 0
        || discovery.revival_scan?.preceding_24h_transactions !== 0
