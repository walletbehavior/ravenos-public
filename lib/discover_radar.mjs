...
export const DISCOVER_CLASSIFIER_VERSION = "2026-08-27.5";
...
function opportunityLane(states, taxonomy, riskFlags, observedAt) {
...
function notabilityContract(row, {
  ...
  const moveTriggers = Object.entries(DISCOVER_NOTABILITY_THRESHOLDS).flatMap(([window, threshold]) => {
    ...
    return [Object.freeze({
      ...
      threshold_multiple: Math.round(thresholdMultiple * 100) / 100,
  ...
  const primaryMove = moveTriggers[0] || null;
  ...
  const accelerationQualified = measurements?.historical_window_coverage?.state === "available"
    && matureSample
  ...
  const lifecycleQualified = observationCount >= 2
    && matureSample
    && [
      "post_dump_resurrection",
      "reclaiming_range",
  ...
  const ravenQualified = raven?.qualified === true && raven?.raven_signal === true;
  ...
  const movePriority = primaryMove
    ? Math.min(120, Math.round(
      50
      + Math.log2(Math.max(1, primaryMove.threshold_multiple)) * 18
  ...
  const extremeMove = Boolean(primaryMove && Math.abs(primaryMove.value_pct) >= 500);
  return Object.freeze({
    ...
    verification_state: qualified
      ...
      : "not_required",
    thresholds: Object.freeze({ ...DISCOVER_NOTABILITY_THRESHOLDS }),
...
function classifyStates(row, measurements, control, cohort) {
  ...
  if (explicitUnavailable) {
    ...
    why = classifierChanged
      ? "The classifier changed while this retained exact market was stale; a current observation is required before assigning its next state."
      : "The exact market remains in Raven's registry after leaving the current provider feed; its latest facts are stale and are not rescored as current.";
  } else if (observationCount < 2) {
    primary = "forming";
    why = "The exact-market registry has one real observation and is waiting for another before assigning a behavioral state.";
...
function classifyOne(row, { timeframe = "5m", nowMs = Date.now() } = {}) {
  ...
  const lane = opportunityLane(states, taxonomy, risks, observedAt);
...
export function validateDiscoverRadarProjection(value, { nowMs = Date.now(), maxAgeSeconds = 3_600 } = {}) {
  ...
  for (const row of value.rows) {
    ...
    if (
      ...
      || Object.entries(DISCOVER_NOTABILITY_THRESHOLDS).some(([window, threshold]) => discovery.notability?.thresholds?.[window] !== threshold)
