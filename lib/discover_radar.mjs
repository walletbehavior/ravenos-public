const CHAINS = Object.freeze(new Set(["solana", "robinhood", "base", "ethereum"]));
...
export const DISCOVER_CLASSIFIER_VERSION = "2026-08-27.1";
...
function freshness(observedAt, nowMs = Date.now(), targetSeconds = 3_600) {
...
function ravenEvidence(row, observedAt) {
  ...
  if (!qualified) {
    return Object.freeze({
      ...
      classifier: null,
      lineage: null,
      why_not_available: "No current exact-market Raven observation cleared the public evidence contract.",
...
function deriveRiskFlags(row, measurements, control) {
  ...
  const marketCap = finite(row?.market?.market_cap_usd ?? row?.market?.fdv_usd);
...
function classifyStates(row, measurements, control, cohort) {
  ...
  if (explicitUnavailable) {
    ...
    primary = PRIMARY_BEHAVIOR_STATES.has(priorState) ? priorState : "forming";
    why = "The candidate remains in Raven's registry after leaving the current provider feed; its latest facts are stale and are not rescored as current.";
  } else if (observationCount < 2) {
    primary = "forming";
    why = "Raven has one real observation and is waiting for another before assigning a behavioral lifecycle state.";
    ...
    why = "The exact pool remains in its initial observed lifecycle without a stronger qualified behavior state yet.";
  }
...
function rawRankIndex(states, measurements, riskFlags, row) {
  ...
  const routeDepth = finite(row?.routeability?.routeable_size_usd) || finite(row?.market?.liquidity_usd) || 0;
...
function cohortKey(row, cohort) {
  return [
    ...
    marketCapBand(row?.market?.market_cap_usd ?? row?.market?.fdv_usd),
...
function stableRiskFlags(flags, observedAt) {
...
function classifyOne(row, { timeframe = "5m", nowMs = Date.now() } = {}) {
  ...
  const primaryBehavior = Object.freeze({
    ...
    availability: states.primary === "forming" ? "insufficient_history" : "available",
  ...
  const base = {
    ...
    discovery: Object.freeze({
      ...
      risk_flags: stableRiskFlags(risks, observedAt),
      raven_evidence_state: raven,
      velocity_state: Object.freeze({
        value: VELOCITY_STATES.has(states.velocity) ? states.velocity : "forming",
        availability: states.velocity === "insufficient_history" ? "insufficient_history" : "available",
      ...
      activity_state: Object.freeze({
        ...
        availability: states.activity === "insufficient_history" ? "insufficient_history" : "available",
        source_scope: "exact_pool_derived_measurements",
        observed_at: observedAt,
        freshness: rowFreshness.state,
        classifier: Object.freeze({ name: DISCOVER_CLASSIFIER_NAME, version: DISCOVER_CLASSIFIER_VERSION }),
      ...
      registry: Object.freeze({
        ...
        admission_reason: cleanText(row?.registry?.admission_reason, 180) || (raven.qualified ? "Exact Raven observation" : "Current provider input admitted for observation"),
      ...
      decision_support: Object.freeze({
        what_changed: cleanText(row?.decision_support?.what_changed || row?.what_changed, 220),
        why_now: cleanText(row?.decision_support?.why_now || states.why, 220),
        what_strengthens: cleanText(row?.decision_support?.what_strengthens, 220) || "Participation and route depth must persist through the next qualified observation.",
        what_weakens: cleanText(row?.decision_support?.what_weakens || row?.risk, 220) || "Flow divergence, thinning liquidity, or loss of exact-market availability weakens the read.",
        next_checkpoint: cleanText(row?.decision_support?.next_checkpoint, 140) || (observationCount < 2 ? "Await the next real observation." : `Re-evaluate after the next ${timeframe} observation.`),
      }),
...
export function buildDiscoverRadarProjection(rows = [], {
  ...
  const normalized = classified.map((row) => {
    ...
    const normalizedVelocityScore = peers.length >= 3 && velocityIndex !== null ? clamp(Math.round((velocityIndex / 100) * 99), 0, 99) : row.discovery.velocity_state.score.score;
    const velocityScore = row.discovery.velocity_state.score.score_cap === null
      ? normalizedVelocityScore
      : finite(normalizedVelocityScore) === null ? null : Math.min(normalizedVelocityScore, row.discovery.velocity_state.score.score_cap);
    const activityScore = peers.length >= 3 && activityIndex !== null ? clamp(Math.round((activityIndex / 100) * 99), 0, 99) : row.discovery.activity_state.score.score;
    return {
      ...row,
      discovery: Object.freeze({
        ...row.discovery,
  ...
  return Object.freeze({
    ...
    state: ["current", "degraded", "forming", "shadow"].includes(sourceState) ? sourceState : "forming",
...
export function validateDiscoverRadarProjection(value, { nowMs = Date.now(), maxAgeSeconds = 3_600 } = {}) {
  if (
    ...
    || !cleanText(value?.classifier?.version, 60)
