...
export const DISCOVER_REVIVAL_SCAN_SCHEMA = "ravenos.discover_revival_scan.v3";
export const DISCOVER_CLASSIFIER_NAME = "raven_behavioral_radar";
export const DISCOVER_CLASSIFIER_VERSION = "2026-09-03.2";
export const DISCOVER_MARKET_FACT_TARGET_SECONDS = 120;
export const DISCOVER_REVIVAL_MIN_AGE_SECONDS = 30 * 86_400;
export const DISCOVER_REVIVAL_MIN_QUIET_SECONDS = 23 * 60 * 60;
export const DISCOVER_REVIVAL_MAX_PRIOR_TRANSACTIONS = 24;
export const DISCOVER_REVIVAL_MIN_RATE_MULTIPLE = 4;
...
function revivalScanEvidence(row, observedAt, freshnessState = "current") {
  const tokenAgeSeconds = finite(row?.market?.token_age_seconds);
  const marketAgeSeconds = tokenAgeSeconds ?? finite(row?.market?.market_age_seconds);
  const ageBasis = tokenAgeSeconds !== null ? "token_creation" : marketAgeSeconds !== null ? "pool_creation" : "unavailable";
  const liquidityUsd = finite(row?.market?.liquidity_usd);
  const latestFlow = flow(row, "5m");
  const currentFlow = flow(row, "1h");
  const dayFlow = flow(row, "24h");
  const currentVolume = metric(row, "volume_usd", "1h");
  const baselineVolume = metric(row, "volume_usd", "24h");
  const priceChangePct = metric(row, "price_change", "1h");
  const transactionRateAcceleration = rateAcceleration(currentFlow.transactions, 60, dayFlow.transactions, 1_440);
  const participantRateAcceleration = rateAcceleration(currentFlow.participants, 60, dayFlow.participants, 1_440);
  const volumeRateAcceleration = rateAcceleration(currentVolume, 60, baselineVolume, 1_440);
  const enoughTransactions = currentFlow.transactions !== null && currentFlow.transactions >= 3;
  const enoughParticipants = currentFlow.participants === null
    ? currentFlow.transactions !== null && currentFlow.transactions >= 5
    : currentFlow.participants >= 2;
  const activityGate = enoughTransactions && enoughParticipants;
  const rollingCountsAvailable = [
    currentFlow.transactions,
    dayFlow.transactions,
  ].every((value) => value !== null);
  const rollingCountsConsistent = rollingCountsAvailable
    && dayFlow.transactions >= currentFlow.transactions;
  const precedingDayTransactions = rollingCountsConsistent
    ? dayFlow.transactions - currentFlow.transactions
    : null;
  const dormancyProven = rollingCountsConsistent && precedingDayTransactions === 0;
  const quietBaselineProven = rollingCountsConsistent
    && precedingDayTransactions <= DISCOVER_REVIVAL_MAX_PRIOR_TRANSACTIONS;
  const priorHourlyRate = precedingDayTransactions === null ? null : precedingDayTransactions / 23;
  const activityReturnedFromZero = precedingDayTransactions === 0 && currentFlow.transactions > 0;
  const reactivationRateMultiple = priorHourlyRate !== null && priorHourlyRate > 0
    ? currentFlow.transactions / priorHourlyRate
    : null;
  const rateBurstProven = activityReturnedFromZero
    || (reactivationRateMultiple !== null && reactivationRateMultiple >= DISCOVER_REVIVAL_MIN_RATE_MULTIPLE);
  const flowSignals = [
    rateBurstProven,
    transactionRateAcceleration !== null && transactionRateAcceleration >= 1,
    participantRateAcceleration !== null && participantRateAcceleration >= 0.5,
    volumeRateAcceleration !== null && volumeRateAcceleration >= 0.5,
  ].filter(Boolean).length;
  const qualified = freshnessState === "current"
    && marketAgeSeconds !== null
    && marketAgeSeconds >= DISCOVER_REVIVAL_MIN_AGE_SECONDS
    && liquidityUsd !== null
    && liquidityUsd > 0
    && activityGate
    && quietBaselineProven
    && rateBurstProven;
  let reasonCode = "qualified";
  if (freshnessState !== "current") reasonCode = "facts_stale";
  else if (marketAgeSeconds === null) reasonCode = "age_unavailable";
  else if (marketAgeSeconds < DISCOVER_REVIVAL_MIN_AGE_SECONDS) reasonCode = "market_too_new";
  else if (liquidityUsd === null) reasonCode = "liquidity_unavailable";
  else if (liquidityUsd <= 0) reasonCode = "liquidity_gone";
  else if (!activityGate) reasonCode = "activity_too_thin";
  else if (!rollingCountsAvailable) reasonCode = "baseline_history_unavailable";
  else if (!rollingCountsConsistent) reasonCode = "rolling_counts_inconsistent";
  else if (!quietBaselineProven) reasonCode = "baseline_not_quiet";
  else if (!rateBurstProven) reasonCode = "rate_burst_unconfirmed";
  return Object.freeze({
    schema_version: DISCOVER_REVIVAL_SCAN_SCHEMA,
    state: qualified ? "qualified" : freshnessState === "current" ? "forming" : "stale",
    qualified,
    reason_code: reasonCode,
    minimum_market_age_seconds: DISCOVER_REVIVAL_MIN_AGE_SECONDS,
    market_age_seconds: marketAgeSeconds,
    age_basis: ageBasis,
    latest_window: "5m",
    current_window: "1h",
    baseline_window: "24h",
    latest_transactions: latestFlow.transactions,
    current_transactions: currentFlow.transactions,
    current_participants: currentFlow.participants,
    preceding_23h_transactions: precedingDayTransactions,
    maximum_preceding_23h_transactions: DISCOVER_REVIVAL_MAX_PRIOR_TRANSACTIONS,
    quiet_baseline_proven: quietBaselineProven,
    dormancy_proven: dormancyProven,
    minimum_quiet_seconds: DISCOVER_REVIVAL_MIN_QUIET_SECONDS,
    activity_returned_from_zero: activityReturnedFromZero,
    reactivation_rate_multiple: reactivationRateMultiple,
    minimum_reactivation_rate_multiple: DISCOVER_REVIVAL_MIN_RATE_MULTIPLE,
    transaction_rate_acceleration: transactionRateAcceleration,
    participant_rate_acceleration: participantRateAcceleration,
    volume_rate_acceleration: volumeRateAcceleration,
    price_change_pct: priceChangePct,
    buy_share: currentFlow.buy_share,
    flow_signal_count: flowSignals,
    single_print_rejected: !activityGate,
    source_scope: "exact_pool_current_hour_vs_preceding_23h_provider_windows",
    observed_at: iso(observedAt),
    freshness: freshnessState,
    historical_series_claimed: false,
    theme_catalyst_identified: false,
    classifier: Object.freeze({ name: DISCOVER_CLASSIFIER_NAME, version: DISCOVER_CLASSIFIER_VERSION }),
  });
}
...
function ravenEvidence(row, observedAt, freshnessState = "current") {
  ...
  if (!qualified) {
    return Object.freeze({
      ...
      classifier: null,
      lineage: null,
      why_not_available: "No current read exists for this exact market yet.",
      provider_rank_used: false,
  ...
  if (freshnessState !== "current") {
    return Object.freeze({
      ...
      why_not_available: "An older exact-market read exists, but current market facts must refresh before it can be shown as current.",
      provider_rank_used: false,
      velocity_score_used: false,
    });
  }
...
function classifyStates(row, measurements, control, cohort) {
  ...
  let primary = "forming";
  let why = "More real market updates are needed before naming a stable pattern.";
  if (explicitUnavailable) {
    ...
    primary = "forming";
    why = "One market update; trend unconfirmed.";
    ...
    why = "The exact post-migration market is expanding from its first recorded state.";
    ...
    why = "Market value expanded materially from its first observation with aligned current flow.";
...
function decisionSupportContract(row, {
  ...
  const stateSupport = {
    ...
    approaching_ath: "Positive velocity and participation persist into the recorded high.",
    ath_breakout: "Price holds above the recorded high with sustained participation and liquidity.",
  ...
  const checkpointFocus = {
    ...
    approaching_ath: "behavior at the recorded high",
    ath_breakout: "acceptance above the recorded high",
...
function classifyOne(row, { timeframe = "5m", nowMs = Date.now() } = {}) {
  ...
  const raven = ravenEvidence(row, observedAt, rowFreshness.state);
  ...
  const revivalScan = revivalScanEvidence(row, observedAt, rowFreshness.state);
...
export function validateDiscoverRadarProjection(value, { nowMs = Date.now(), maxAgeSeconds = 3_600 } = {}) {
  ...
  for (const row of value.rows) {
    ...
    if (
      ...
      || !["qualified", "forming", "stale"].includes(discovery.revival_scan?.state)
      || discovery.revival_scan?.freshness !== expectedFactFreshness
      || discovery.revival_scan?.historical_series_claimed !== false
      || discovery.revival_scan?.theme_catalyst_identified !== false
      || (discovery.revival_scan?.qualified === true && (
        expectedFactFreshness !== "current"
        || discovery.revival_scan?.single_print_rejected !== false
        || discovery.revival_scan?.quiet_baseline_proven !== true
        || finite(discovery.revival_scan?.preceding_23h_transactions) === null
        || discovery.revival_scan?.preceding_23h_transactions > DISCOVER_REVIVAL_MAX_PRIOR_TRANSACTIONS
        || (
          discovery.revival_scan?.activity_returned_from_zero !== true
          && finite(discovery.revival_scan?.reactivation_rate_multiple) < DISCOVER_REVIVAL_MIN_RATE_MULTIPLE
        )
        || discovery.revival_scan?.market_age_seconds < DISCOVER_REVIVAL_MIN_AGE_SECONDS
      ))
      ...
      || (expectedFactFreshness === "stale" && (
        discovery.raven_evidence_state?.qualified === true
        || discovery.raven_evidence_state?.raven_signal === true
      ))
    ) return null;
