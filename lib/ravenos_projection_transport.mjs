...
export const PUBLIC_PROJECTION_TRANSPORT_POLICY = Object.freeze({
  ...
  key_fresh_transport_seconds: Object.freeze({
    opportunities: 10,
  }),
  opportunity_radar_rescue_seconds: 120,
...
function freshTransportSeconds(key) {
  return Number(PUBLIC_PROJECTION_TRANSPORT_POLICY.key_fresh_transport_seconds[String(key || "")])
    || PUBLIC_PROJECTION_TRANSPORT_POLICY.fresh_transport_seconds;
}

function opportunityRadarIsCurrent(result, nowMs) {
  const generatedMs = Date.parse(String(result?.payload?.data?.discovery_radar?.generated_at || ""));
  if (!Number.isFinite(generatedMs) || generatedMs > nowMs + 300_000) return false;
  const ageSeconds = Math.max(0, Math.floor((nowMs - generatedMs) / 1_000));
  return ageSeconds <= PUBLIC_PROJECTION_TRANSPORT_POLICY.opportunity_radar_rescue_seconds;
}

function cachedProjectionIsCurrentEnough(result, key, nowMs) {
  return key !== "opportunities" || opportunityRadarIsCurrent(result, nowMs);
}
...
export async function loadResilientPublicProjection({
  ...
  const freshTransportMs = freshTransportSeconds(key) * 1_000;
  if (
    cached
    && nowMs - cached.cachedAt <= freshTransportMs
    && cachedProjectionIsCurrentEnough(cached.result, key, nowMs)
  ) {
    return {
      ...cached.result,
