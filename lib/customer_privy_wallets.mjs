...
function config(env = {}) {
  ...
  const identityPublicJwk = parseJwk(env.RAVENOS_PRIVY_IDENTITY_PUBLIC_JWK);
  return Object.freeze({
    ...
    identity_public_jwk: identityPublicJwk,
    configured: Boolean(active && appId && clean(env.RAVENOS_PRIVY_CLIENT_ID, 180) && publicJwk && privateJwk && identityPublicJwk && env.RAVENOS_CUSTOMER_DB?.prepare),
  });
}
...
export async function verifyPrivyIdentityToken(token, cfg, { now = Math.floor(Date.now() / 1000) } = {}) {
  ...
  const jwk = cfg.identity_public_jwk;
  if (!jwk || (jwk.kid && jwk.kid !== header.kid)) throw new Error("privy_identity_key_unavailable");
