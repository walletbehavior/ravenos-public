...
const RESERVED_USERNAMES = new Set([
  "admin", "administrator", "help", "moderator", "official", "raven", "ravenos",
  "root", "security", "staff", "support", "system", "team", "undefined", "null",
]);
...
export function normalizeRavenUsername(value) {
  const username = clean(value, 80).replace(/^@/, "").toLowerCase();
  if (!/^[a-z][a-z0-9_]{2,23}$/.test(username)) throw new Error("username_invalid");
...
function storedUsername(value) {
  try {
    return normalizeRavenUsername(value);
  } catch {
    return null;
  }
}

function workosClientId(value) {
  const clientId = clean(value, 200);
  return /^client_[A-Za-z0-9_]+$/.test(clientId) ? clientId : "";
}

function workosApiKey(value) {
  const apiKey = clean(value, 300);
  return /^sk_[A-Za-z0-9_-]+$/.test(apiKey) ? apiKey : "";
}
...
export function customerIdentityConfigured(env = {}) {
  ...
  return env.RAVENOS_CUSTOMER_ACCOUNTS_ENABLE === "1"
    ...
    && Boolean(workosClientId(env.WORKOS_CLIENT_ID))
    && Boolean(workosApiKey(env.WORKOS_API_KEY))
...
export function publicCustomerIdentityConfig(env = {}, requestUrl = "https://ravenos.xyz/") {
  ...
  return {
    ...
    methods: {
      ...
      email: available,
      password: available,
      magic_auth: available,
      passkey: false,
    },
    account_model: {
      principal: "ravenos_account",
      public_identity: "user_selected_username",
      identity_provider_name_is_public: false,
...
function safeReturnTo(value) {
  const path = clean(value || "/account/", 1600);
...
function publicProfile(row) {
  const username = storedUsername(row.username);
  return {
    username,
    username_required: !username,
    display_name: username ? `@${username}` : "Raven user",
  ...
  return {
    ...
    async resolveOrCreateIdentity(identity) {
      const find = () => db.prepare(`
        SELECT c.credential_id, c.user_id, u.state, u.primary_email, u.username, u.display_name, u.created_at AS user_created_at
      ...
      await db.batch([
        db.prepare("UPDATE ravenos_users SET primary_email = ?, display_name = NULL, updated_at = ?, last_authenticated_at = ? WHERE user_id = ?")
          .bind(identity.email, identity.now, identity.now, row.user_id),
      ...
      return { ...row, primary_email: identity.email, display_name: null, created };
    },
    ...
    async findSession(sessionVerifier) {
      return db.prepare(`
        SELECT s.*, u.state AS user_state, u.primary_email, u.username, u.display_name, u.created_at AS user_created_at
    ...
    async updateUsername(userId, username, now) {
      try {
        const result = await db.prepare(`
          UPDATE ravenos_users
          SET username = ?, display_name = NULL, updated_at = ?
          WHERE user_id = ? AND state = 'active'
        `).bind(username, now, userId).run();
        if (Number(result?.meta?.changes || 0) !== 1) throw new Error("account_unavailable");
        const row = await db.prepare(`
          SELECT user_id, state, primary_email, username, display_name, created_at AS user_created_at
          FROM ravenos_users WHERE user_id = ? LIMIT 1
        `).bind(userId).first();
...
export async function consumeCustomerRateLimit({
  ...
  limit,
  include_network: includeNetwork = false,
} = {}) {
  if (!store?.rateLimit || !request) throw new Error("customer_rate_limit_dependencies_required");
  const normalizedAction = clean(action, 80).toLowerCase();
  const normalizedScope = clean(scope, 40).toLowerCase();
  const normalizedSubject = clean(subject, 300);
  const pepper = clean(env.RAVENOS_AUTH_HASH_PEPPER, 300);
  const normalizedWindow = Number(windowSeconds);
  const normalizedLimit = Number(limit);
  if (!/^[a-z0-9_:-]+$/.test(normalizedAction) || !/^[a-z0-9_:-]+$/.test(normalizedScope) || !normalizedSubject || !pepper) {
    throw new Error("customer_rate_limit_scope_invalid");
  }
  if (!Number.isSafeInteger(normalizedWindow) || normalizedWindow < 30 || normalizedWindow > 86_400) {
    throw new Error("customer_rate_limit_window_invalid");
  }
  if (!Number.isSafeInteger(normalizedLimit) || normalizedLimit < 1 || normalizedLimit > 10_000) {
    throw new Error("customer_rate_limit_limit_invalid");
  }
  const material = JSON.stringify({
    schema_version: "ravenos.customer_rate_key.v1",
    action: normalizedAction,
    scope: normalizedScope,
    subject: normalizedSubject,
    network: includeNetwork ? requestIpPrefix(request) : null,
  });
  const rateKey = await hmacSha256(pepper, material);
  const result = await store.rateLimit({
    rateKey,
    action: `${normalizedAction}:${normalizedScope}`,
    now: Number(now),
  ...
  return Object.freeze({
    ...
    retry_after_seconds: Math.max(1, Number(result?.retry_after_seconds || normalizedWindow)),
    scope: normalizedScope,
  });
}
...
function authorizationUrl(env, { state, codeChallenge, provider, intent }) {
  ...
  url.searchParams.set("client_id", workosClientId(env.WORKOS_CLIENT_ID));
...
async function exchangeManagedIdentity(env, state, code, fetchImpl) {
  const response = await fetchImpl("https://api.workos.com/user_management/authenticate", {
    ...
    body: JSON.stringify({
      client_id: workosClientId(env.WORKOS_CLIENT_ID),
      client_secret: workosApiKey(env.WORKOS_API_KEY),
  ...
  return {
    ...
    email,
    // Provider names are identity-provider metadata, not a RavenOS public
    // identity. In particular, a family name must never become a default
...
export async function authorizeCustomerApiRequest(request, env = {}, deps = {}, {
  require_csrf: requireCsrf = false,
} = {}) {
  if (!customerIdentityConfigured(env)) return { response: unavailable(env, request) };
  const origin = originFromEnv(env);
  if (!origin || new URL(request.url).origin !== origin) {
    return { response: json({ ok: false, error: "authenticated_origin_required", canonical_origin: origin }, { status: 409 }) };
  }
  ...
  try {
    if (requireCsrf) {
      ...
      return {
        principal: Object.freeze({
          user_id: required.session.row.user_id,
          session_public_id: required.session.row.session_public_id,
          authenticated_at: Number(required.session.row.authenticated_at),
        }),
        store,
        now,
    ...
    return {
      principal: Object.freeze({
        user_id: session.row.user_id,
        session_public_id: session.row.session_public_id,
        authenticated_at: Number(session.row.authenticated_at),
      }),
      store,
      now,
    ...
    return { response: json({ ok: false, error: "account_service_unavailable" }, { status: 503 }) };
  }
}
...
async function handleUsernameUpdate(request, env, store, now) {
  const required = await requireMutationSession(request, env, store, now);
  if (required.response) return required.response;
  let body;
  let username;
  try {
    body = await boundedBody(request, 2_048);
    username = normalizeRavenUsername(body.username);
  } catch (error) {
    const reason = error?.message === "username_reserved" ? "username_reserved" : "username_invalid";
    return json({ ok: false, error: reason }, { status: 400 });
  }
  const limited = await consumeCustomerRateLimit({
    store,
    env,
    request,
    action: "username_update",
    scope: "user",
    subject: required.session.row.user_id,
    now,
    window_seconds: USERNAME_CHANGE_WINDOW_SECONDS,
    limit: USERNAME_CHANGE_LIMIT,
  });
  if (!limited.allowed) {
    return json({ ok: false, error: "username_update_rate_limited" }, {
      status: 429,
      headers: { "retry-after": String(limited.retry_after_seconds) },
    });
  }
  let account;
  try {
    account = await store.updateUsername(required.session.row.user_id, username, now);
  } catch (error) {
    return json({ ok: false, error: error?.message === "username_unavailable" ? "username_unavailable" : "account_unavailable" }, {
      status: error?.message === "username_unavailable" ? 409 : 503,
    });
  }
  await store.recordEvent({
  ...
  return json({
    ok: true,
    schema_version: "ravenos.account_username.v1",
...
export async function routeCustomerIdentity(request, env = {}, deps = {}) {
  ...
  try {
    ...
    if (path === "/api/v1/account/username" && request.method === "PUT") return handleUsernameUpdate(request, env, store, now);
    ...
    return json({ ok: false, error: "method_not_allowed" }, { status: 405, headers: { allow: path === "/api/v1/account/username" ? "PUT" : path === "/api/v1/auth/start" || path === "/api/v1/auth/logout" ? "POST" : "GET" } });
  } catch {
