...
function availableContext(instrument) {
  return {
    ...
    plan_preview: {
      ...
      personalized: false,
      executable: false,
      note: "No current frozen research context is available for this instrument.",
    },
  };
}
...
function readText(context) {
  if (!context.context_available) {
    return {
      ...
      summary: "Live market data remains available, but Raven has no current frozen observation for this instrument.",
  ...
  return {
    ...
    summary: `${context.pressure_state || "Mixed pressure"} accompanied a frozen ${direction} research observation. ${sampleText}`,
    why_raven_noticed: context.why_raven_noticed,
...
export function buildPerpTerminalContext({ publicPerpsPayload, marketPayload, symbol } = {}) {
  ...
  return {
    ...
    plan_preview: {
      ...
      label: "Research plan preview",
      execution_available: false,
      disclaimer: "Historical excursions and frozen research context are not personalized targets, stops, or orders.",
    },
