...
export function normalizeHyperliquidPerps(payload, { now = new Date() } = {}) {
  ...
  return universe.map((meta, index) => {
    if (!meta || meta.isDelisted === true) return null;
    ...
    return {
      ...
      premium,
      asset_index: index,
      sz_decimals: Number.isSafeInteger(Number(meta.szDecimals)) ? Number(meta.szDecimals) : null,
      ...
      basis: premium,
      assetIndex: index,
      szDecimals: Number.isSafeInteger(Number(meta.szDecimals)) ? Number(meta.szDecimals) : null,
