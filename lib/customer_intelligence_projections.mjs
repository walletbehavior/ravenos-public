...
const MAX_FREE_PERP_ROWS = 6;
const MAX_FREE_PARTICIPANT_ROWS = 6;
...
function participantPublicLabel(value, max = 240) {
  return cleanText(value, max)
    .replace(/jupiter[ _-]+velocity/gi, "high-velocity tokens");
}

function participantPublicText(value, max = 320) {
  return cleanText(value, max)
    .replace(/\bJupiter Velocity\b/gi, "High-velocity token")
    .replace(/jupiter[ _-]+velocity/gi, "high-velocity token");
}
...
function perpsFreeMarketRow(row) {
  const market = perpsMarketRow(row);
  if (!market) return null;
  return Object.freeze({
    instrument_id: market.instrument_id,
    symbol: market.symbol,
    venue: market.venue,
    instrument_group: market.instrument_group,
    funding_rate: market.funding_rate,
    funding_regime: market.funding_regime,
...
function exactSelectedMarket(rows, instrumentId, { free = false } = {}) {
  ...
  const source = rows.find((row) => row.instrument_id === exact) || null;
  const market = source && free ? perpsFreeMarketRow(source) : source;
  return Object.freeze({
  ...
  return {
    ...
    base: {
      ...
      overview: Object.freeze({
        ...
        participant_context: Object.freeze({
          state: cleanText(data.actor_evidence?.actor_evidence_state || data.actor_evidence_state, 80) || "unavailable",
          freshness: cleanText(data.actor_evidence?.actor_evidence_freshness || data.actor_evidence_freshness, 40) || "unavailable",
          observed_at: timestamp(data.actor_evidence?.observed_at),
          privacy: "aggregate_status_only",
        }),
      }),
      selected_market: exactSelectedMarket(selectedRows, selectedInstrumentId, { free: true }),
      market_overview: Object.freeze(topVolume.slice(0, MAX_FREE_PERP_ROWS).map(perpsFreeMarketRow).filter(Boolean)),
...
function participantBasicRow(row) {
  ...
  const capBand = participantPublicLabel(row.cap_band, 80);
  ...
  return Object.freeze({
    ...
    interpretation: participantPublicText(row.plain_language_summary, 320) || "Interpretation unavailable.",
  });
}
...
function participantAdvancedRow(row) {
  ...
  const fallbackCohortId = `${basic.chain}_${basic.capitalization_band}_${basic.window}`.toLowerCase().replace(/[^a-z0-9_-]+/g, "_").slice(0, 120);
  const suppliedCohortId = participantPublicLabel(row.heatmap_id, 120).toLowerCase().replace(/[^a-z0-9_-]+/g, "_");
  const cohortId = /^[a-z0-9][a-z0-9_-]{2,119}$/.test(suppliedCohortId) ? suppliedCohortId : fallbackCohortId;
  const behaviorState = cleanText(row.derived_state, 80) || "outcomes unclear";
  const averageOutcome = cleanText(row.avg_outcome, 60) || "unavailable";
  const associationDirection = /rewarding|constructive|positive/i.test(`${behaviorState} ${averageOutcome}`)
    ? "favorable"
    : /punishing|adverse|negative/i.test(`${behaviorState} ${averageOutcome}`)
      ? "unfavorable"
      : "mixed";
  const searchTerms = [...new Set([
    basic.chain,
    basic.capitalization_band,
    basic.window,
    basic.participation_trend,
    behaviorState,
    averageOutcome,
    ...array(row.search_terms, 16).map((value) => participantPublicLabel(value, 60)),
  ].map((value) => cleanText(value, 60).toLowerCase()).filter(Boolean))].slice(0, 20);
  return Object.freeze({
    ...basic,
    cohort_id: cohortId,
    ...
    outcome_score: unitRate(row.outcome_score),
    search_terms: Object.freeze(searchTerms),
    ...
    average_outcome_classification: averageOutcome,
...
function participantBase(payload, { delivery = {}, chains = null } = {}) {
  const data = publicData(payload);
  if (!data || !Array.isArray(data.rows)) throw new Error("participant_public_projection_invalid");
  const chainAllowlist = Array.isArray(chains)
    ? new Set(chains.map((value) => cleanText(value, 40).toLowerCase()).filter(Boolean))
    : null;
  const sourceRows = chainAllowlist?.size
    ? data.rows.filter((row) => chainAllowlist.has(cleanText(row?.chain, 40).toLowerCase()))
    : data.rows;
  const rows = array(sourceRows, MAX_PRO_PARTICIPANT_ROWS).map(participantBasicRow).filter(Boolean);
  return {
    data,
    rows,
    base: {
      ...
      participation_overview: Object.freeze(rows.slice(0, MAX_FREE_PARTICIPANT_ROWS)),
      limitations: Object.freeze({
        ...
        causality: "not_established_by_descriptive_associations",
        calibrated_probability: "not_claimed",
      }),
    },
  };
}
  ...
  return Object.freeze({
    ...
    advanced: Object.freeze({
      ...
      filters: Object.freeze({
        ...
        participation_trends: Object.freeze(uniqueValues(matrix, "participation_trend")),
        behavior_states: Object.freeze(uniqueValues(matrix, "behavior_state")),
        outcome_classes: Object.freeze(uniqueValues(matrix, "average_outcome_classification")),
        confidences: Object.freeze(uniqueValues(matrix, "confidence")),
      }),
...
export const CustomerIntelligenceProjectionContract = Object.freeze({
  schema_version: PROJECTION_SCHEMA,
  free_limits: Object.freeze({ perps_markets: MAX_FREE_PERP_ROWS, participant_conditions: MAX_FREE_PARTICIPANT_ROWS }),
  pro_limits: Object.freeze({ perps_table_rows: MAX_PRO_PERP_ROWS, participant_conditions: MAX_PRO_PARTICIPANT_ROWS }),
  free_field_contracts: Object.freeze({
    perps_market: Object.freeze([
      "instrument_id",
      "symbol",
      "venue",
      "instrument_group",
    ...
    participant_condition: Object.freeze([
      "chain",
      "capitalization_band",
      "window",
      "participation_trend",
  ...
  pro_field_contracts: Object.freeze({
    perps_market: Object.freeze([
      "instrument_id",
      "symbol",
      "venue",
