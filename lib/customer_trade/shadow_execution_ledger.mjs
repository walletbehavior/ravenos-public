...
import {
  SHADOW_FEE_CALCULATION_VERSION,
  SHADOW_FEE_SCENARIO_MATRIX_SCHEMA,
...
export const SHADOW_FEE_EVIDENCE_SCHEMA = "ravenos.shadow_fee_evidence.v1";
...
function integerString(value, field) {
  const clean = String(value ?? "").trim();
  if (!/^\d+$/.test(clean) || BigInt(clean) > BigInt(Number.MAX_SAFE_INTEGER)) fail(`${field}_invalid`);
  return clean;
}
...
export function createShadowRouteObservation({
  ...
  const expiresAt = unix(roundTrip?.expires_at || entry?.expires_at, "expires_at");
  ...
  return Object.freeze({
    ...
    quote_only_round_trip_loss_pct: finite(roundTrip.quote_only_round_trip_loss_pct, "quote_only_round_trip_loss_pct", { optional: true, minimum: -10_000, maximum: 100_000 }),
...
export function createShadowFeeEvidenceRows({ observation, matrix } = {}) {
  const observationId = cleanText(observation?.observation_id, "observation_id", 100);
  if (matrix?.schema_version !== SHADOW_FEE_SCENARIO_MATRIX_SCHEMA) fail("shadow_fee_matrix_required");
  if (matrix.route_observation_id !== observationId) fail("shadow_fee_observation_mismatch");
  const observedAt = Math.trunc(finite(observation?.observed_at, "observed_at", { minimum: 0 }));
  const retentionExpiresAt = Math.trunc(finite(observation?.retention_expires_at, "retention_expires_at", { minimum: observedAt + 1 }));
  const rows = Array.isArray(matrix.rows) ? matrix.rows : [];
  if (rows.length !== ShadowFeeScenarioBps.length) fail("shadow_fee_matrix_incomplete");
  const seen = new Set();
  return rows.map((row) => {
    const scenarioBps = Math.trunc(finite(row?.scenario_bps, "scenario_bps", { minimum: 0, maximum: 10_000 }));
    if (!ShadowFeeScenarioBps.includes(scenarioBps) || seen.has(scenarioBps)) fail("shadow_fee_scenario_invalid");
    seen.add(scenarioBps);
    const entry = row.entry_fee || {};
    const exit = row.exit_fee || {};
    const outcome = row.outcome || {};
    const entryCollectionState = cleanText(row.entry_collection?.collection_state, "entry_collection_state", 48, { lower: true });
    const exitCollectionState = cleanText(row.exit_collection?.collection_state, "exit_collection_state", 48, { lower: true });
    const evidenceId = `shfev_${digest(`${observationId}|${scenarioBps}|${SHADOW_FEE_CALCULATION_VERSION}`).slice(0, 32)}`;
    return Object.freeze({
      fee_evidence_id: evidenceId,
      schema_version: SHADOW_FEE_EVIDENCE_SCHEMA,
      observation_id: observationId,
      calculation_version: SHADOW_FEE_CALCULATION_VERSION,
      scenario_bps: scenarioBps,
      entry_basis_usdc_micros: integerString(entry.fee_basis_usdc_micros, "entry_basis_usdc_micros"),
      entry_fee_usdc_micros: integerString(entry.hypothetical_fee_usdc_micros, "entry_fee_usdc_micros"),
      exit_basis_usdc_micros: integerString(exit.fee_basis_usdc_micros, "exit_basis_usdc_micros"),
      exit_fee_usdc_micros: integerString(exit.hypothetical_fee_usdc_micros, "exit_fee_usdc_micros"),
      round_trip_fee_usdc_micros: integerString(outcome.round_trip_raven_fee_usdc_micros, "round_trip_fee_usdc_micros"),
      gross_terminal_usdc_micros: integerString(outcome.gross_terminal_usdc_micros, "gross_terminal_usdc_micros"),
      minimum_net_terminal_usdc_micros: integerString(outcome.minimum_net_terminal_usdc_micros, "minimum_net_terminal_usdc_micros"),
      round_trip_friction_excluding_raven_pct: finite(outcome.round_trip_friction_excluding_raven_pct, "round_trip_friction_excluding_raven_pct", { optional: true, minimum: -10_000, maximum: 100_000 }),
      round_trip_friction_including_raven_pct: finite(outcome.round_trip_friction_including_raven_pct, "round_trip_friction_including_raven_pct", { optional: true, minimum: -10_000, maximum: 100_000 }),
      entry_collection_state: entryCollectionState,
      exit_collection_state: exitCollectionState,
      collection_evidence_complete: optionalBoolean(outcome.fee_collection_complete),
      actual_collection_authorized: 0,
...
function rowObservation(row = {}) {
  const sourceAmount = row.source_amount_usdc == null ? null : Number(row.source_amount_usdc);
  const currentExit = row.current_exit_usdc == null ? null : Number(row.current_exit_usdc);
  const quoteOnlyLoss = row.quote_only_round_trip_loss_pct == null
    && Number.isFinite(sourceAmount) && sourceAmount > 0 && Number.isFinite(currentExit)
    ? ((sourceAmount - currentExit) / sourceAmount) * 100
    : row.quote_only_round_trip_loss_pct == null ? null : Number(row.quote_only_round_trip_loss_pct);
  return {
    ...row,
...
export function createD1ShadowExecutionLedgerStore(db) {
  ...
  return Object.freeze({
    ...
    async recordFeeEvidence(records = []) {
      let inserted = 0;
      for (const record of records.slice(0, ShadowFeeScenarioBps.length)) {
        const result = await ledgerDb.prepare(`
          INSERT OR IGNORE INTO ravenos_shadow_fee_evidence (
            fee_evidence_id, schema_version, observation_id, calculation_version, scenario_bps,
            entry_basis_usdc_micros, entry_fee_usdc_micros, exit_basis_usdc_micros, exit_fee_usdc_micros,
            round_trip_fee_usdc_micros, gross_terminal_usdc_micros, minimum_net_terminal_usdc_micros,
            round_trip_friction_excluding_raven_pct, round_trip_friction_including_raven_pct,
            entry_collection_state, exit_collection_state, collection_evidence_complete,
            actual_collection_authorized, actual_collected_usdc_micros, observed_at, retention_expires_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(
          record.fee_evidence_id, SHADOW_FEE_EVIDENCE_SCHEMA, record.observation_id, record.calculation_version,
          record.scenario_bps, record.entry_basis_usdc_micros, record.entry_fee_usdc_micros,
          record.exit_basis_usdc_micros, record.exit_fee_usdc_micros, record.round_trip_fee_usdc_micros,
          record.gross_terminal_usdc_micros, record.minimum_net_terminal_usdc_micros,
          record.round_trip_friction_excluding_raven_pct, record.round_trip_friction_including_raven_pct,
          record.entry_collection_state, record.exit_collection_state, record.collection_evidence_complete,
          0, null, record.observed_at, record.retention_expires_at,
        ).run();
        inserted += Number(result?.meta?.changes || 0);
      }
      return inserted;
    },
    async recentFeeEvidence(since, limit = ShadowRouteLedgerLimits.maximum_summary_rows * ShadowFeeScenarioBps.length, now = Math.floor(Date.now() / 1_000)) {
      const result = await ledgerDb.prepare(`
        SELECT f.* FROM ravenos_shadow_fee_evidence f
        JOIN ravenos_shadow_route_observations o ON o.observation_id = f.observation_id
        WHERE f.observed_at >= ? AND f.retention_expires_at > ? AND o.retention_expires_at > ?
        ORDER BY f.observed_at DESC, f.fee_evidence_id ASC LIMIT ?
      `).bind(since, now, now, Math.min(ShadowRouteLedgerLimits.maximum_summary_rows * ShadowFeeScenarioBps.length, Math.max(1, Number(limit) || 1))).all();
      if (!Array.isArray(result?.results)) fail("shadow_fee_evidence_query_failed");
      return result.results;
    },
...
export function buildShadowRouteReadinessProjection(observations = [], checkpoints = [], { generated_at, window_seconds = ShadowRouteLedgerLimits.summary_window_seconds, fee_evidence = [] } = {}) {
  ...
  const slices = [...groups.values()].map((group) => {
    ...
    return {
      ...
      median_quote_only_round_trip_loss_pct: median(group.rows.map((row) => row.quote_only_round_trip_loss_pct)),
    };
  ...
  const feeStudy = ShadowFeeScenarioBps.map((scenarioBps) => {
    const current = fee_evidence.filter((row) => Number(row.scenario_bps) === scenarioBps);
    return {
      scenario_bps: scenarioBps,
      route_samples: new Set(current.map((row) => String(row.observation_id))).size,
      median_entry_fee_usdc: median(current.map((row) => Number(row.entry_fee_usdc_micros) / 1_000_000)),
      median_round_trip_fee_usdc: median(current.map((row) => Number(row.round_trip_fee_usdc_micros) / 1_000_000)),
      median_round_trip_friction_excluding_raven_pct: median(current.map((row) => row.round_trip_friction_excluding_raven_pct)),
      median_round_trip_friction_including_raven_pct: median(current.map((row) => row.round_trip_friction_including_raven_pct)),
      collection_evidence_complete_pct: percent(current.filter((row) => Number(row.collection_evidence_complete) === 1).length, current.length),
  ...
  return Object.freeze({
    ...
    median_quote_only_round_trip_loss_pct: median(rows.map((row) => row.quote_only_round_trip_loss_pct)),
...
export async function loadShadowRouteReadiness(store, { now = Math.floor(Date.now() / 1_000), window_seconds = ShadowRouteLedgerLimits.summary_window_seconds } = {}) {
  ...
  const feeEvidence = typeof store.recentFeeEvidence === "function"
    ? await store.recentFeeEvidence(since, ShadowRouteLedgerLimits.maximum_summary_rows * ShadowFeeScenarioBps.length, now)
    : [];
  return buildShadowRouteReadinessProjection(observations, checkpoints, { generated_at: now * 1_000, window_seconds, fee_evidence: feeEvidence });
}
