...
export function createD1ShadowExecutionLedgerStore(db) {
  ...
  return Object.freeze({
    ...
    async purgeExpired(now) {
      const result = await db.prepare("DELETE FROM ravenos_shadow_route_observations WHERE retention_expires_at <= ?").bind(now).run();
      return Number(result?.meta?.changes || 0);
    },
    async acquireLease(token, now) {
      await db.prepare(`INSERT OR IGNORE INTO ravenos_shadow_evaluator_lease (lease_key, lease_token, lease_expires_at, revision, updated_at) VALUES ('universal_shadow_v1', NULL, NULL, 1, ?)`)
        .bind(now).run();
      await db.prepare(`
      ...
      const row = await db.prepare("SELECT lease_token FROM ravenos_shadow_evaluator_lease WHERE lease_key = 'universal_shadow_v1'").first();
...
export async function loadShadowRouteReadiness(store, { now = Math.floor(Date.now() / 1_000), window_seconds = ShadowRouteLedgerLimits.summary_window_seconds } = {}) {
  ...
  const [observations, checkpoints] = await Promise.all([
    store.recentObservations(since, ShadowRouteLedgerLimits.maximum_summary_rows, now),
    store.recentCheckpoints(since, ShadowRouteLedgerLimits.maximum_summary_rows, now),
  ]);
