...
export function resolveCustomerLiveExecutionGate(env = {}, principal = null, { nowSeconds = Math.floor(Date.now() / 1000) } = {}) {
  ...
  const bscSourceReady = sourceBoundaryReady && CustomerLiveExecutionAuthorization.evm_wallet_transaction_submission;
  const baseSourceReady = sourceBoundaryReady && CustomerLiveExecutionAuthorization.evm_wallet_transaction_submission;
  const ethereumSourceReady = sourceBoundaryReady && CustomerLiveExecutionAuthorization.evm_wallet_transaction_submission;
  const sourceReady = hyperliquidSourceReady
    || solanaSourceReady
    || robinhoodSourceReady
    || bscSourceReady
    || baseSourceReady
    || ethereumSourceReady;
  ...
  const bscEnabled = configured && bscSourceReady && enabled(env.RAVENOS_CUSTOMER_TRADE_BSC_LIVE_ENABLE);
  const baseEnabled = configured && baseSourceReady && enabled(env.RAVENOS_CUSTOMER_TRADE_BASE_LIVE_ENABLE);
  const ethereumEnabled = configured && ethereumSourceReady && enabled(env.RAVENOS_CUSTOMER_TRADE_ETHEREUM_LIVE_ENABLE);
  ...
  return Object.freeze({
    ...
    chains: Object.freeze({
      ...
      bsc: Object.freeze({
        source_ready: bscSourceReady,
        enabled: bscEnabled,
        available_to_principal: bscSourceReady && bscEnabled && principalAllowed,
        wallet_signing: "eip1193_transaction_in_connected_wallet",
        submission: "wallet_signed_direct_to_bnb_chain",
      }),
      base: Object.freeze({
        source_ready: baseSourceReady,
        enabled: baseEnabled,
        available_to_principal: baseSourceReady && baseEnabled && principalAllowed,
        wallet_signing: "eip1193_transaction_in_connected_wallet",
        submission: "wallet_signed_direct_to_base",
      }),
      ethereum: Object.freeze({
        source_ready: ethereumSourceReady,
        enabled: ethereumEnabled,
        available_to_principal: ethereumSourceReady && ethereumEnabled && principalAllowed,
        wallet_signing: "eip1193_transaction_in_connected_wallet",
        submission: "wallet_signed_direct_to_ethereum",
      }),
    }),
...
export function publicCustomerLiveExecutionCapabilities(env = {}) {
  ...
  return Object.freeze({
    ...
    chains: Object.freeze({
      ...
      bsc: Object.freeze({ source_ready: gate.chains.bsc.source_ready, enabled: gate.chains.bsc.enabled }),
      base: Object.freeze({ source_ready: gate.chains.base.source_ready, enabled: gate.chains.base.enabled }),
      ethereum: Object.freeze({ source_ready: gate.chains.ethereum.source_ready, enabled: gate.chains.ethereum.enabled }),
    }),
  });
}
