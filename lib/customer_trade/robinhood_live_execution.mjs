...
import {
  ...
  ROBINHOOD_ZERO_X_UNSIGNED_QUOTE_SCHEMA,
} from "./robinhood_zero_x_live_execution.mjs";

export const ROBINHOOD_LIVE_TICKET_SCHEMA = "ravenos.robinhood_live_ticket.v1";
export const ROBINHOOD_LIVE_CLIENT_REPORT_SCHEMA = "ravenos.robinhood_live_client_report.v1";
export const ROBINHOOD_LIVE_RECONCILIATION_SCHEMA = "ravenos.robinhood_live_reconciliation.v1";
...
function address(value, field, { native = false } = {}) {
  ...
  if (!ADDRESS_RE.test(normalized) || normalized === ZERO_ADDRESS || (!native && normalized === NATIVE_ETH)) fail(`${field}_invalid`);
  return normalized;
}
...
function quoteIdentity(quote, field) {
  if (quote?.schema_version !== ROBINHOOD_ZERO_X_UNSIGNED_QUOTE_SCHEMA
    || quote.chain_id !== ROBINHOOD_ZERO_X_CHAIN_ID
    || quote.canonical_chain_id !== ROBINHOOD_ZERO_X_CANONICAL_CHAIN_ID
    || quote.ok !== true
...
function exactMarket(value = {}) {
  const poolAddress = address(value.pool_address, "robinhood_pool_address");
  const tokenAddress = address(value.token_address, "robinhood_token_address");
  const quoteAddress = address(value.quote_address, "robinhood_quote_address");
  const instrumentId = clean(value.instrument_id, 180);
  if (instrumentId !== `robinhood:pool:${poolAddress}`) fail("robinhood_exact_market_identity_mismatch");
  const side = clean(value.side, 8).toLowerCase();
  if (!new Set(["buy", "sell"]).has(side)) fail("robinhood_side_invalid");
  return freeze({
...
function feeFromQuote(quote, accountingAsset) {
  const fee = quote?.fee || {};
  if (typeof fee.enabled !== "boolean") fail("robinhood_fee_evidence_invalid");
  if (!fee.enabled) {
    if (Number(fee.fee_bps) !== 0 || String(fee.amount) !== "0" || fee.token !== null || fee.recipient !== null) {
      fail("robinhood_fee_evidence_invalid");
    }
    return freeze({
      enabled: false,
      fee_bps: 0,
  ...
  if (!Number.isSafeInteger(feeBps) || feeBps < 1 || feeBps > 1_000) fail("robinhood_fee_bps_invalid");
  const token = address(fee.token, "robinhood_fee_token", { native: true });
  const recipient = address(fee.recipient, "robinhood_fee_recipient");
  const amount = integerString(fee.amount, "robinhood_fee_amount", { positive: true });
  return freeze({
    enabled: true,
    fee_bps: feeBps,
    token,
...
export function createRobinhoodLiveTicket(input = {}, { now = Date.now(), ttl_ms: ttlMs = 8_000 } = {}) {
  assertRobinhoodZeroXQuoteFresh(input.entry_quote, { now });
  const market = exactMarket(input.exact_market);
  const entry = quoteIdentity(input.entry_quote, "robinhood_entry_quote");
  if (entry.expires_at_ms <= now + 1_000) fail("robinhood_entry_quote_expired");
  const walletAddress = address(input.wallet_address, "robinhood_wallet_address");
  if (entry.taker !== walletAddress) fail("robinhood_entry_wallet_mismatch");
  const accounting = freeze({
    asset_address: address(input.accounting?.asset_address, "robinhood_accounting_asset"),
    symbol: clean(input.accounting?.symbol, 16).toUpperCase(),
    decimals: Number(input.accounting?.decimals),
    notional_base_units: integerString(input.accounting?.notional_base_units, "robinhood_accounting_notional", { positive: true }),
    maximum_notional_base_units: integerString(input.accounting?.maximum_notional_base_units, "robinhood_accounting_maximum", { positive: true }),
  });
  if (accounting.symbol !== "USDG" || accounting.decimals !== 6) fail("robinhood_accounting_asset_not_supported");
  if (BigInt(accounting.notional_base_units) > BigInt(accounting.maximum_notional_base_units)) fail("robinhood_live_notional_out_of_bounds");
  if (entry.sell_token !== address(input.entry_quote?.exact_binding?.sell_token, "robinhood_entry_sell_token", { native: true })) {
    fail("robinhood_entry_identity_mismatch");
  }
  if (market.side === "buy" && entry.buy_token !== market.token_address) fail("robinhood_entry_identity_mismatch");
  if (market.side === "sell" && entry.sell_token !== market.token_address) fail("robinhood_entry_identity_mismatch");
  if (input.entry_quote?.wallet_handoff_eligible !== true || input.entry_quote?.state !== "awaiting_wallet_signature" || !input.entry_quote?.unsigned_transaction) {
    fail("robinhood_entry_not_wallet_handoff_eligible");
  }
  const reviewedTransactionHash = digest(input.entry_quote.reviewed_transaction_hash, "robinhood_reviewed_transaction_hash");
  const fee = feeFromQuote(input.entry_quote, accounting.asset_address);

  let exitProof = null;
  if (market.side === "buy") {
    const exit = quoteIdentity(input.exit_quote, "robinhood_exit_quote");
    if (exit.expires_at_ms <= now + 1_000) fail("robinhood_exit_quote_expired");
    if (exit.taker !== walletAddress
      ...
      fail("robinhood_exit_quote_identity_mismatch");
    }
    ...
    if (blockers.some((reason) => !new Set(["allowance_required", "insufficient_balance"]).has(reason))
      ...
      fail("robinhood_exit_quote_unresolved");
    }
    exitProof = freeze({
      verified: true,
  ...
  if (expiresAtMs <= now + 1_000) fail("robinhood_live_ticket_expired");
  ...
  const binding = {
    ...
    accounting,
    fee,
    exit_quote_hash: exitProof?.quote_hash || null,
    expires_at: new Date(expiresAtMs).toISOString(),
  };
  const ticket = freeze({
    ok: true,
    schema_version: ROBINHOOD_LIVE_TICKET_SCHEMA,
    ticket_id: ticketId,
    ...
    transaction: freeze({
      reviewed_transaction_hash: reviewedTransactionHash,
      to: address(transaction.to, "robinhood_transaction_to"),
      maximum_gas: integerString(transaction.gas, "robinhood_transaction_gas", { positive: true }),
      quoted_gas_price: transaction.gas_price === undefined
        ? null
        : integerString(transaction.gas_price, "robinhood_transaction_gas_price"),
      quoted_maximum_fee_per_gas: transaction.max_fee_per_gas === undefined
        ? null
        : integerString(transaction.max_fee_per_gas, "robinhood_transaction_max_fee_per_gas"),
      quoted_maximum_priority_fee_per_gas: transaction.max_priority_fee_per_gas === undefined
        ? null
        : integerString(transaction.max_priority_fee_per_gas, "robinhood_transaction_max_priority_fee_per_gas"),
      value: integerString(transaction.value, "robinhood_transaction_value"),
    ...
    execution_boundary: freeze({
      ...
      submission_path: "wallet_direct_to_robinhood_chain",
...
export function normalizeRobinhoodClientExecutionReport(input = {}, expected = {}) {
  if (expected?.schema_version !== ROBINHOOD_LIVE_TICKET_SCHEMA) fail("robinhood_live_ticket_invalid");
  const ticketId = clean(input.ticket_id, 100);
  if (ticketId !== expected.ticket_id || !ticketId.startsWith("lex_")) fail("robinhood_execution_ticket_mismatch");
  const walletAddress = address(input.wallet_address, "robinhood_execution_wallet");
  if (walletAddress !== expected.wallet_address) fail("robinhood_execution_wallet_mismatch");
  const reviewedHash = clean(input.reviewed_transaction_hash, 64).toLowerCase();
  if (reviewedHash !== expected.transaction?.reviewed_transaction_hash) fail("robinhood_execution_reviewed_transaction_mismatch");
  const transactionHash = clean(input.transaction_hash, 66).toLowerCase();
  if (!TX_HASH_RE.test(transactionHash) || /^0x0{64}$/.test(transactionHash)) fail("robinhood_execution_transaction_hash_invalid");
  return freeze({
    schema_version: ROBINHOOD_LIVE_CLIENT_REPORT_SCHEMA,
    ticket_id: ticketId,
    venue: "zero_x",
    chain_id: ROBINHOOD_ZERO_X_CHAIN_ID,
    wallet_address: walletAddress,
...
function sumTransfers(logs, { token, from = null, to = null }) {
  if (token === NATIVE_ETH) return null;
  let total = 0n;
  for (const log of Array.isArray(logs) ? logs : []) {
    ...
    let observedFrom;
    let observedTo;
    observedFrom = logAddressTopic(topics[1], "robinhood_transfer_from");
    observedTo = logAddressTopic(topics[2], "robinhood_transfer_to");
    if (from && observedFrom !== from) continue;
    if (to && observedTo !== to) continue;
    total += logAmountWord(log.data, "robinhood_transfer_amount");
  }
  return total;
}

export async function reconcileRobinhoodExecution(input = {}, { rpc_client: rpcClient, now = Date.now(), minimum_confirmations: minimumConfirmations = 1 } = {}) {
  const ticket = input.ticket || {};
  const report = input.client_report || {};
  if (ticket.schema_version !== ROBINHOOD_LIVE_TICKET_SCHEMA || report.schema_version !== ROBINHOOD_LIVE_CLIENT_REPORT_SCHEMA) {
    fail("robinhood_reconciliation_input_invalid");
  }
  if (ticket.ticket_id !== report.ticket_id || ticket.wallet_address !== report.wallet_address) fail("robinhood_reconciliation_identity_mismatch");
  if (!rpcClient?.request) fail("robinhood_reconciliation_rpc_unavailable");
  ...
  if (!transaction || !receipt) {
    return freeze({
      schema_version: ROBINHOOD_LIVE_RECONCILIATION_SCHEMA,
      state: "indeterminate",
      transaction_hash: transactionHash,
  ...
  const txFrom = address(transaction.from, "robinhood_chain_transaction_from");
  const txTo = address(transaction.to, "robinhood_chain_transaction_to");
  ...
  if (clean(transaction.hash, 66).toLowerCase() !== transactionHash
    ...
    || hexQuantity(transaction.value, "robinhood_chain_transaction_value") !== BigInt(ticket.transaction.value)
    || hexQuantity(transaction.gas, "robinhood_chain_transaction_gas") > BigInt(ticket.transaction.maximum_gas)) {
    fail("robinhood_chain_transaction_binding_mismatch");
  }
  const receiptHash = clean(receipt.transactionHash, 66).toLowerCase();
  if (receiptHash !== transactionHash
    || address(receipt.from, "robinhood_receipt_from") !== ticket.wallet_address
    || address(receipt.to, "robinhood_receipt_to") !== ticket.transaction.to) {
    fail("robinhood_chain_receipt_binding_mismatch");
  }
  const status = hexQuantity(receipt.status, "robinhood_receipt_status");
  if (status === 0n) {
    return freeze({
      schema_version: ROBINHOOD_LIVE_RECONCILIATION_SCHEMA,
  ...
  if (status !== 1n) fail("robinhood_receipt_status_invalid");
  const receiptBlock = hexQuantity(receipt.blockNumber, "robinhood_receipt_block");
  const headBlock = hexQuantity(headResult.result, "robinhood_head_block");
  if (headBlock < receiptBlock) fail("robinhood_receipt_ahead_of_chain");
  ...
  } else if (ticket.fee.token === NATIVE_ETH) {
    feeCollection = {
      state: "expected",
      token: NATIVE_ETH,
  ...
  const gasUsed = hexQuantity(receipt.gasUsed, "robinhood_receipt_gas_used");
  const effectiveGasPrice = hexQuantity(receipt.effectiveGasPrice || transaction.gasPrice || "0x0", "robinhood_receipt_effective_gas_price");
  ...
  return freeze({
    ...
    evidence: {
      ...
      finality_state: "canonical_l2_confirmation",
...
function exactBaseUnitsToDisplayNumber(value, decimals) {
  ...
  const raw = integerString(value, "robinhood_accounting_display_amount");
...
export function createD1RobinhoodLiveExecutionStore(db) {
  ...
  return freeze({
    async createTicket({ prepared, user_id: userId, now_seconds: nowSeconds }) {
      ...
      if (ticket?.schema_version !== ROBINHOOD_LIVE_TICKET_SCHEMA) fail("robinhood_live_ticket_invalid");
      noTransactionMaterial(ticket);
      ...
      await db.prepare(`
        ...
        VALUES (?, ?, ?, 'zero_x', 'robinhood', ?, ?, ?, 'market', NULL, ?, ?, NULL, ?, ?, ?, ?,
                'awaiting_wallet_signature', ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?)
      `).bind(
        ticket.ticket_id,
        ROBINHOOD_LIVE_TICKET_SCHEMA,
        userId,
    ...
    async findTicket(executionId, userId) {
      const row = await db.prepare(`
        ...
        WHERE execution_id = ? AND user_id = ? AND venue = 'zero_x' AND chain_namespace = 'robinhood'
    ...
    async recordClientReport({ record, user_id: userId, now_seconds: nowSeconds }) {
      ...
      `).bind(record.transaction_hash, nowSeconds, record.ticket_id, userId, nowSeconds).run();
      if (changes(result) !== 1) fail("robinhood_execution_ticket_not_reportable");
    ...
    async reconcile({ execution_id: executionId, user_id: userId, reconciliation, now_seconds: nowSeconds }) {
      ...
      if (changes(result) !== 1) fail("robinhood_execution_not_reconcilable");
