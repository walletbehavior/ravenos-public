...
function normalizeSummary(clearinghouse = {}) {
  ...
  const cross = clearinghouse.crossMarginSummary && typeof clearinghouse.crossMarginSummary === "object"
    ...
    : {};
  return {
    account_value_usdc: rounded(margin.accountValue),
    withdrawable_usdc: rounded(clearinghouse.withdrawable),
    position_notional_usdc: rounded(margin.totalNtlPos),
    margin_used_usdc: rounded(margin.totalMarginUsed),
...
export function createHyperliquidAccountSnapshot(input = {}, { observedAt = new Date().toISOString() } = {}) {
  ...
  const summary = normalizeSummary(clearinghouse);
