...
import { createSourceWalletId } from "./source_wallet_chain_identity.mjs";
...
export async function runSourceWalletObserverBatch(store, deps = {}, {
  ...
  const totals = {
    ...
    copyability_probes_evaluated: 0,
    copyability_observations_recorded: 0,
    copyability_observation_duplicates: 0,
    crowding_evaluations: 0,
    crowding_observations_recorded: 0,
    crowding_observation_duplicates: 0,
  ...
  await runPool(jobs, concurrency, async (job) => {
    ...
    try {
      ...
      totals.copyability_probes_evaluated += Math.max(0, Number(fanoutResult?.copyability_probe_count) || 0);
      totals.copyability_observations_recorded += Math.max(0, Number(fanoutResult?.copyability_observation_count) || 0);
      totals.copyability_observation_duplicates += Math.max(0, Number(fanoutResult?.copyability_duplicate_count) || 0);
      totals.crowding_evaluations += Math.max(0, Number(fanoutResult?.crowding_evaluation_count) || 0);
      totals.crowding_observations_recorded += Math.max(0, Number(fanoutResult?.crowding_observation_count) || 0);
      totals.crowding_observation_duplicates += Math.max(0, Number(fanoutResult?.crowding_duplicate_count) || 0);
