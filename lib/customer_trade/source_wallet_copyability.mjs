...
import { buildSourceWalletCopyCrowdingPublicSummary } from "./source_wallet_copy_crowding.mjs";
...
export const SOURCE_WALLET_COPY_PLAYBOOK_SCHEMA = "ravenos.source_wallet_copy_playbook.v1";
...
const COPYABILITY_MARKET_REGIMES = Object.freeze([
  Object.freeze({
    dimension: "market_cap_usd",
    label: "Detected market cap",
    context_field: "market_cap_usd",
    buckets: Object.freeze([
      Object.freeze({ id: "under_200k", label: "Under $200K", minimum: 0, maximum: 200_000 }),
      Object.freeze({ id: "200k_750k", label: "$200K–$750K", minimum: 200_000, maximum: 750_000 }),
      Object.freeze({ id: "750k_2m", label: "$750K–$2M", minimum: 750_000, maximum: 2_000_000 }),
      Object.freeze({ id: "2m_plus", label: "$2M+", minimum: 2_000_000, maximum: null }),
    ]),
  }),
  Object.freeze({
    dimension: "liquidity_usd",
    label: "Detected liquidity",
    context_field: "liquidity_usd",
    buckets: Object.freeze([
      Object.freeze({ id: "under_25k", label: "Under $25K", minimum: 0, maximum: 25_000 }),
      Object.freeze({ id: "25k_100k", label: "$25K–$100K", minimum: 25_000, maximum: 100_000 }),
      Object.freeze({ id: "100k_500k", label: "$100K–$500K", minimum: 100_000, maximum: 500_000 }),
      Object.freeze({ id: "500k_plus", label: "$500K+", minimum: 500_000, maximum: null }),
    ]),
  }),
  Object.freeze({
    dimension: "pair_age_seconds",
    label: "Selected pair age",
    context_field: "pair_age_seconds",
    buckets: Object.freeze([
      Object.freeze({ id: "under_5m", label: "Under 5m", minimum: 0, maximum: 300 }),
      Object.freeze({ id: "5m_1h", label: "5m–1h", minimum: 300, maximum: 3_600 }),
      Object.freeze({ id: "1h_24h", label: "1h–24h", minimum: 3_600, maximum: 86_400 }),
      Object.freeze({ id: "1d_plus", label: "1d+", minimum: 86_400, maximum: null }),
    ]),
  }),
]);
...
function compactRegimeSnapshot(decisions, generatedAt, orderSizeUsdc) {
  const snapshot = buildCopyabilitySnapshot(decisions, {
    generated_at: generatedAt,
    order_size_usdc: orderSizeUsdc,
  });
  return freeze({
    state: snapshot.state,
    confidence: snapshot.confidence,
    prospective_sample_count: snapshot.prospective_sample_count,
    executable_count: snapshot.executable_count,
    refusal_count: snapshot.refusal_count,
    entry_executable_pct: snapshot.components.entry_executable_pct,
    exit_executable_pct: snapshot.components.exit_executable_pct,
    policy_pass_pct: snapshot.components.policy_pass_pct,
    median_entry_degradation_bps: snapshot.components.median_entry_degradation_bps,
    median_round_trip_friction_pct: snapshot.components.median_round_trip_friction_pct,
    dominant_refusal: snapshot.dominant_refusal,
  });
}

function buildCopyabilityMarketRegimes(rows, generatedAt, referenceOrderSizeUsdc) {
  const referenceRows = (Array.isArray(rows) ? rows : []).filter((row) => row.standard_order_size_usdc === referenceOrderSizeUsdc);
  const dimensions = COPYABILITY_MARKET_REGIMES.map((definition) => {
    const observed = referenceRows.filter((row) => Number.isFinite(row.market_context?.[definition.context_field]));
    const buckets = definition.buckets.map((bucket, bucketIndex) => {
      const bucketRows = observed.filter((row) => {
        const value = row.market_context[definition.context_field];
        return value >= bucket.minimum && (bucket.maximum === null || value < bucket.maximum);
      });
      const snapshot = compactRegimeSnapshot(bucketRows.map(decisionView).filter(Boolean), generatedAt, referenceOrderSizeUsdc);
      return freeze({
        bucket_id: bucket.id,
        bucket_label: bucket.label,
        lower_bound_inclusive: bucket.minimum,
        upper_bound_exclusive: bucket.maximum,
        bucket_order: bucketIndex,
        ...snapshot,
      });
    });
    const qualified = buckets.filter((row) => row.prospective_sample_count >= 20 && Number.isFinite(row.policy_pass_pct));
    const forming = buckets.filter((row) => row.prospective_sample_count > 0 && Number.isFinite(row.policy_pass_pct));
    const representativePool = qualified.length ? qualified : forming;
    const representative = [...representativePool].sort((left, right) => (
      (right.policy_pass_pct ?? -1) - (left.policy_pass_pct ?? -1)
      || right.prospective_sample_count - left.prospective_sample_count
      || left.bucket_order - right.bucket_order
    ))[0] || null;
    return freeze({
      dimension: definition.dimension,
      label: definition.label,
      state: !observed.length ? "insufficient_evidence" : qualified.length ? "available" : "forming",
      reference_signal_count: referenceRows.length,
      context_observation_count: observed.length,
      context_coverage_pct: coveragePercent(observed.length, referenceRows.length),
      minimum_prospective_sample_count: 20,
      representative_bucket: representative,
      representative_selection_basis: representative
        ? qualified.length
          ? "highest_policy_pass_with_minimum_sample"
          : "highest_policy_pass_while_forming"
        : "unavailable",
      buckets,
    });
  });
  const qualifiedBuckets = dimensions.flatMap((dimension) => dimension.buckets.map((bucket) => ({
    dimension: dimension.dimension,
    dimension_label: dimension.label,
    ...bucket,
  }))).filter((row) => row.prospective_sample_count >= 20 && Number.isFinite(row.policy_pass_pct));
  const ranked = [...qualifiedBuckets].sort((left, right) => (
    right.policy_pass_pct - left.policy_pass_pct
    || right.prospective_sample_count - left.prospective_sample_count
  ...
  return freeze({
    ...
    regime_version: 1,
    state: !referenceRows.length
      ? "insufficient_evidence"
      : dimensions.some((row) => row.state === "available")
        ? "available"
        : dimensions.some((row) => row.context_observation_count > 0)
          ? "forming"
          : "insufficient_evidence",
    evidence_scope: "prospective_reference_size_raven_detection_context",
    reference_order_size_usdc: referenceOrderSizeUsdc,
    reference_signal_count: referenceRows.length,
    minimum_prospective_sample_count: 20,
    dimensions,
    strongest_observed_policy_fit: ranked[0] || null,
    weakest_observed_policy_fit: ranked.length ? ranked.at(-1) : null,
    exact_source_pool_claimed: false,
...
function buildCopyabilitySizeStress(rows, bySize, prospectiveSignalCount) {
  const observations = Array.isArray(rows) ? rows : [];
  const curve = (Array.isArray(bySize) ? bySize : []).map((snapshot) => {
    const sizeRows = observations.filter((row) => row.standard_order_size_usdc === snapshot.order_size_usdc);
    const decisions = sizeRows.map(decisionView).filter(Boolean);
    return freeze({
      order_size_usdc: snapshot.order_size_usdc,
      prospective_sample_count: snapshot.prospective_sample_count,
      evidence_qualified: snapshot.prospective_sample_count >= 20
        && Number.isFinite(snapshot.components?.policy_pass_pct),
      policy_pass_pct: snapshot.components?.policy_pass_pct ?? null,
      median_entry_price_impact_bps: median(decisions.map((row) => row.entry?.price_impact_bps), 0),
      median_exit_price_impact_bps: median(decisions.map((row) => row.reverse_exit?.price_impact_bps), 0),
      median_round_trip_friction_pct: snapshot.components?.median_round_trip_friction_pct ?? null,
      dominant_refusal: snapshot.dominant_refusal || null,
    });
  });
  const byEvent = new Map();
  for (const row of observations) {
    if (!row?.source_event_id || !RavenCopyStandardOrderSizesUsdc.includes(row.standard_order_size_usdc)) continue;
    const sizes = byEvent.get(row.source_event_id) || new Set();
    sizes.add(row.standard_order_size_usdc);
    byEvent.set(row.source_event_id, sizes);
  }
  const fullLadderSignals = [...byEvent.values()].filter((sizes) => (
    RavenCopyStandardOrderSizesUsdc.every((size) => sizes.has(size))
  )).length;
  const qualified = curve.filter((row) => row.evidence_qualified);
  const reversals = qualified.slice(1).filter((row, index) => (
    row.policy_pass_pct > qualified[index].policy_pass_pct + 5
  )).length;
  const firstBelowMajority = qualified.find((row) => row.policy_pass_pct < 50) || null;
  let contiguousMajority = null;
  for (const row of curve) {
    if (!row.evidence_qualified || row.policy_pass_pct < 50) break;
    contiguousMajority = row;
  }
  const highestQualified = qualified.at(-1) || null;
  const state = !prospectiveSignalCount
    ? "insufficient_evidence"
    : qualified.length < 2
      ? "forming"
      : reversals
        ? "mixed_evidence"
        : qualified[0].policy_pass_pct < 50
          ? "constrained_at_smallest_tested"
          : firstBelowMajority
            ? "size_sensitive"
            : "resilient_through_largest_tested";
  return freeze({
    ...
    minimum_prospective_sample_count: 20,
    standard_order_sizes_usdc: RavenCopyStandardOrderSizesUsdc,
    full_ladder_signal_count: fullLadderSignals,
    full_ladder_coverage_pct: coveragePercent(fullLadderSignals, prospectiveSignalCount),
    qualified_size_count: qualified.length,
    highest_qualified_size_usdc: highestQualified?.order_size_usdc ?? null,
    largest_contiguous_size_with_majority_policy_pass_usdc: contiguousMajority?.order_size_usdc ?? null,
    first_qualified_size_below_majority_policy_pass_usdc: firstBelowMajority?.order_size_usdc ?? null,
    non_monotonic_reversal_count: reversals,
    by_size: curve,
    evidence_scope: "isolated_exact_route_quote_ladder",
    concurrent_follower_demand_measured: false,
    aggregate_follower_capital_included: false,
    subscriber_count_included: false,
    batched_execution_assumed: false,
    isolated_quotes_presented_as_concurrent_fills: false,
    liquidity_capacity_claimed: false,
  });
}

export function buildSourceWalletCopyPlaybook({
  prospective_signal_count: prospectiveSignalCount = 0,
  snapshot = null,
  by_size: bySize = [],
  copy_diagnosis: diagnosis = null,
  size_stress: sizeStress = null,
  market_regimes: marketRegimes = null,
  prospective_outcomes: outcomes = null,
  crowding = null,
} = {}) {
  const signalCount = integer(prospectiveSignalCount, "source_wallet_copy_playbook_signal_count", {
    maximum: SourceWalletCopyabilityLimits.maximum_observations_per_source_profile,
  });
  const sizeRows = (Array.isArray(bySize) ? bySize : [])
    .filter((row) => RavenCopyStandardOrderSizesUsdc.includes(Number(row?.order_size_usdc)))
    .map((row) => ({
      order_size_usdc: Number(row.order_size_usdc),
      prospective_sample_count: Number(row.prospective_sample_count || 0),
      policy_pass_pct: Number.isFinite(row.components?.policy_pass_pct) ? Number(row.components.policy_pass_pct) : null,
      evidence_qualified: Number(row.prospective_sample_count || 0) >= 20
        && Number.isFinite(row.components?.policy_pass_pct),
    }))
    .sort((left, right) => left.order_size_usdc - right.order_size_usdc);
  const sampledSizes = sizeRows.filter((row) => row.prospective_sample_count > 0);
  const qualifiedSizes = sizeRows.filter((row) => row.evidence_qualified);
  const majoritySize = Number(sizeStress?.largest_contiguous_size_with_majority_policy_pass_usdc);
  const firstWeakSize = Number(sizeStress?.first_qualified_size_below_majority_policy_pass_usdc);
  const majorityRow = sizeRows.find((row) => row.order_size_usdc === majoritySize) || null;
  const firstWeakRow = sizeRows.find((row) => row.order_size_usdc === firstWeakSize) || null;
  const stressState = String(sizeStress?.state || "insufficient_evidence");
  const state = !signalCount
    ? "insufficient_evidence"
    : qualifiedSizes.length < 2
      ? "forming"
      : stressState === "constrained_at_smallest_tested"
        ? "constrained"
        : "available";
  const headlineCode = state === "insufficient_evidence"
    ? "NO_PROSPECTIVE_EVIDENCE"
    : state === "forming"
      ? "NEEDS_MORE_ROUTE_SAMPLES"
      : stressState === "size_sensitive"
        ? "ROUTES_WEAKEN_ABOVE_SIZE"
        : stressState === "constrained_at_smallest_tested"
          ? "CONSTRAINED_AT_SMALLEST_TESTED"
          : stressState === "mixed_evidence"
            ? "ROUTE_EVIDENCE_MIXED"
            : "ROUTES_SURVIVE_THROUGH_LARGEST_TESTED";

  const strongestMarket = marketRegimes?.strongest_observed_policy_fit || null;
  const marketContextCount = (Array.isArray(marketRegimes?.dimensions) ? marketRegimes.dimensions : [])
    .reduce((maximum, dimension) => Math.max(maximum, Number(dimension?.context_observation_count || 0)), 0);
  const marketFit = strongestMarket
    ? {
        state: "available",
        dimension: strongestMarket.dimension,
        dimension_label: strongestMarket.dimension_label,
        bucket_id: strongestMarket.bucket_id,
        bucket_label: strongestMarket.bucket_label,
        prospective_sample_count: strongestMarket.prospective_sample_count,
        policy_pass_pct: strongestMarket.policy_pass_pct,
        dominant_refusal: strongestMarket.dominant_refusal || null,
      }
    : {
        state: marketContextCount > 0 ? "forming" : "insufficient_evidence",
        dimension: null,
        dimension_label: null,
        bucket_id: null,
        bucket_label: null,
        prospective_sample_count: marketContextCount,
        policy_pass_pct: null,
        dominant_refusal: null,
      };

  const outcomeReference = outcomes?.reference || null;
  const outcomeCount = Number(outcomeReference?.checkpoint_count || 0);
  const routePersistence = {
    state: outcomeCount >= 20 ? "available" : outcomeCount > 0 ? "forming" : "insufficient_evidence",
    order_size_usdc: Number(outcomeReference?.order_size_usdc || SourceWalletCopyabilityLimits.reference_order_size_usdc),
    horizon_seconds: Number(outcomeReference?.horizon_seconds || 3_600),
    checkpoint_count: outcomeCount,
    route_persistence_pct: Number.isFinite(outcomeReference?.route_persistence_pct)
      ? Number(outcomeReference.route_persistence_pct)
      : null,
    median_follower_return_pct: Number.isFinite(outcomeReference?.median_follower_return_pct)
      ? Number(outcomeReference.median_follower_return_pct)
      : null,
    follower_capture_sample_count: Number(outcomeReference?.follower_capture_sample_count || 0),
    median_follower_capture_ratio_pct: Number.isFinite(outcomeReference?.median_follower_capture_ratio_pct)
      ? Number(outcomeReference.median_follower_capture_ratio_pct)
      : null,
  };

  const dominantRefusal = diagnosis?.reference_dominant_refusal || snapshot?.dominant_refusal || null;
  const sizeBreakdown = (Array.isArray(sizeStress?.by_size) ? sizeStress.by_size : [])
    .find((row) => Number(row?.order_size_usdc) === firstWeakSize)?.dominant_refusal || null;
  const crowdingConstraint = crowding?.dominant_constraint || null;
  const leadingConstraint = dominantRefusal
    ? {
        state: "observed",
        scope: "reference_order_exact_routes",
        reason_code: dominantRefusal.reason_code,
        observation_count: Number(dominantRefusal.count || 0),
        pct_of_signals: Number.isFinite(dominantRefusal.pct_of_signals) ? Number(dominantRefusal.pct_of_signals) : null,
      }
    : sizeBreakdown
      ? {
          state: "observed",
          scope: "first_below_majority_size_exact_routes",
          order_size_usdc: Number.isFinite(firstWeakSize) && firstWeakSize > 0 ? firstWeakSize : null,
          reason_code: sizeBreakdown.reason_code,
          observation_count: Number(sizeBreakdown.count || 0),
          pct_of_signals: Number.isFinite(sizeBreakdown.pct_of_signals) ? Number(sizeBreakdown.pct_of_signals) : null,
        }
    : crowdingConstraint
      ? {
          state: "observed",
          scope: "privacy_qualified_aggregate_route_stress",
          reason_code: crowdingConstraint.reason_code,
          observation_count: Number(crowdingConstraint.signal_count || 0),
          pct_of_signals: Number.isFinite(crowdingConstraint.pct_of_eligible_signals)
            ? Number(crowdingConstraint.pct_of_eligible_signals)
            : null,
        }
      : {
          state: signalCount ? "none_observed" : "insufficient_evidence",
          scope: null,
          reason_code: null,
          observation_count: 0,
          pct_of_signals: null,
  ...
  return freeze({
    schema_version: SOURCE_WALLET_COPY_PLAYBOOK_SCHEMA,
    playbook_version: 1,
    state,
    headline_code: headlineCode,
    prospective_signal_count: signalCount,
    minimum_prospective_sample_count: 20,
    reference_order_size_usdc: SourceWalletCopyabilityLimits.reference_order_size_usdc,
    reference_policy_pass_pct: Number.isFinite(snapshot?.components?.policy_pass_pct)
      ? Number(snapshot.components.policy_pass_pct)
      : null,
    size_window: {
      state: stressState,
      smallest_sampled_size_usdc: sampledSizes[0]?.order_size_usdc ?? null,
      largest_sampled_size_usdc: sampledSizes.at(-1)?.order_size_usdc ?? null,
      evidence_qualified_size_count: qualifiedSizes.length,
      largest_contiguous_majority_pass_size_usdc: Number.isFinite(majoritySize) && majoritySize > 0 ? majoritySize : null,
      policy_pass_pct_at_largest_majority_size: majorityRow?.policy_pass_pct ?? null,
      first_below_majority_size_usdc: Number.isFinite(firstWeakSize) && firstWeakSize > 0 ? firstWeakSize : null,
      policy_pass_pct_at_first_below_majority_size: firstWeakRow?.policy_pass_pct ?? null,
      isolated_exact_route_quotes_only: true,
      position_size_recommendation: false,
      liquidity_capacity_claimed: false,
    },
    strongest_observed_market_fit: marketFit,
    route_persistence: routePersistence,
    leading_constraint: leadingConstraint,
    evidence_scope: "prospective_exact_entry_reverse_exit_and_policy_observations",
    unavailable_decisions_dropped: false,
...
export function buildSourceWalletCopyabilityMatrix(
  ...
  const copyDiagnosis = buildCopyabilityDiagnosis(bySize, SourceWalletCopyabilityLimits.reference_order_size_usdc);
  const sizeStress = buildCopyabilitySizeStress(selectedRows, bySize, selected?.prospective_signal_count || 0);
  const marketRegimes = buildCopyabilityMarketRegimes(selectedRows, generated, SourceWalletCopyabilityLimits.reference_order_size_usdc);
  const crowding = buildSourceWalletCopyCrowdingPublicSummary(crowdingObservations);
  const copyPlaybook = buildSourceWalletCopyPlaybook({
    prospective_signal_count: selected?.prospective_signal_count || 0,
    snapshot: selected?.snapshot || null,
    by_size: bySize,
    copy_diagnosis: copyDiagnosis,
    size_stress: sizeStress,
  ...
  return freeze({
    ...
    limitations: [
      ...
      "Aggregate route stress is privacy-thresholded research; it does not disclose follower demand or promise simultaneous allocation or fills.",
...
export const SourceWalletCopyabilityContract = Object.freeze({
  schema_version: SOURCE_WALLET_COPYABILITY_MATRIX_SCHEMA,
  copy_playbook_schema_version: SOURCE_WALLET_COPY_PLAYBOOK_SCHEMA,
  ...
  pair_age_used_as_token_age: false,
  isolated_route_size_stress_presented_as_crowding_capacity: false,
  shared_quote_work: true,
  subscriber_proportional_quote_work: false,
  prospective_only: true,
  source_performance_substitution: false,
  copy_playbook_deterministic: true,
  copy_playbook_financial_advice: false,
  live_copy: false,
  aggregate_crowding_capacity_claimed: false,
  signing: false,
  broadcasting: false,
  custody: false,
  fee_collection: false,
