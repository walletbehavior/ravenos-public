...
export function createRavenCopyDecision(input = {}, { now = Date.now() } = {}) {
  ...
  const suppliedMarket = input.market_context && typeof input.market_context === "object" ? input.market_context : {};
  const marketContext = {
    chain: suppliedMarket.chain === "solana" ? "solana" : null,
    pair_address: /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(String(suppliedMarket.pair_address || "")) ? suppliedMarket.pair_address : null,
    token_address: /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(String(suppliedMarket.token_mint || "")) ? suppliedMarket.token_mint : null,
    quote_address: /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(String(suppliedMarket.quote_address || "")) ? suppliedMarket.quote_address : null,
    venue: clean(suppliedMarket.venue, "market_venue", 80, { optional: true }) || null,
    observed_at: suppliedMarket.observed_at ? timestamp(suppliedMarket.observed_at, "market_observed_at") : null,
  };
  const exactTerminalMarket = marketContext.chain === "solana"
    && marketContext.pair_address
    && marketContext.token_address === sourceEvent.economic.destination_asset?.mint
    && marketContext.quote_address
    ? marketContext
    : null;
  ...
  return freeze({
    ...
    terminal_handoff: exactTerminalMarket ? {
      state: "user_review_available",
      chain: exactTerminalMarket.chain,
      instrument_id: `solana:pool:${exactTerminalMarket.pair_address}`,
      identity_scope: "exact_pool",
      pair_address: exactTerminalMarket.pair_address,
      token_address: exactTerminalMarket.token_address,
      quote_address: exactTerminalMarket.quote_address,
      side: "buy",
      amount_usdc: policy.sizing.fixed_usdc,
      venue: exactTerminalMarket.venue,
      market_observed_at: exactTerminalMarket.observed_at,
      wallet_signature_required: true,
      automatic_submission: false,
    } : {
      state: "exact_market_unavailable",
      wallet_signature_required: true,
      automatic_submission: false,
    },
    policy,
