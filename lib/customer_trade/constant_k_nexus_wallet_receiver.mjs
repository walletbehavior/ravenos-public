...
export const ConstantKNexusReceiverLimits = Object.freeze({
  ...
  maximum_watches: SourceWalletTransportLimits.maximum_watches_per_run,
});
...
async function runTransportChunks({ universe, events, ingestDelivery, generatedAt }) {
  const maximumChunkSize = Math.max(1, Math.floor(
    SourceWalletTransportLimits.maximum_stream_references_per_run / Math.max(1, universe.length),
  ));
  const rows = events.length ? events : [[]];
  const chunks = events.length
    ? Array.from({ length: Math.ceil(events.length / maximumChunkSize) }, (_, index) => events.slice(index * maximumChunkSize, (index + 1) * maximumChunkSize))
    : rows;
  const counts = {
...
export async function runConstantKNexusWalletReceiverCycle({
  ...
  const universe = normalizeSourceWalletWatchUniverse(watches, { maximum_watches: ConstantKNexusReceiverLimits.maximum_watches });
