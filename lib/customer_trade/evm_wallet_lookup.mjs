...
function basicProfile({
  ...
  const transferTransactions = new Map();
  for (const event of events) {
    const reference = event.chain_evidence.transaction_reference;
    const row = transferTransactions.get(reference) || { inbound: new Set(), outbound: new Set() };
    ...
    transferTransactions.set(reference, row);
  }
  const routeDecodeCandidateTransactions = [...transferTransactions.values()].filter((row) =>
    row.inbound.size > 0
    && row.outbound.size > 0
