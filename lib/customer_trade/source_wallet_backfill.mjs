...
export function createD1SourceWalletBackfillStore(db, { record_events: recordEvents } = {}) {
  ...
  return freeze({
    async enqueueJob({ address, provider = "configured_solana_rpc", now = Date.now() } = {}) {
      const job = createSourceWalletBackfillJob({ address, provider, requested_at: new Date(now).toISOString() });
      ...
      `).bind(job.job_id, job.source_wallet_id, job.provider, seconds, seconds, seconds).run();
    ...
    async leaseJobs({ worker_id: workerId, now, limit, lease_seconds: leaseSeconds }) {
      ...
      const candidates = await db.prepare(`
        ...
        ORDER BY next_attempt_at ASC, created_at ASC, job_id ASC LIMIT ?
    ...
    async health({ now = Date.now() } = {}) {
      const rows = await db.prepare(`
        ...
        FROM ravenos_source_wallet_backfill_jobs GROUP BY state
      `).all();
      return freeze({
        schema_version: "ravenos.source_wallet_backfill_health.v1",
        observed_at: new Date(now).toISOString(),
        queue: (rows?.results || []).map((row) => ({ state: row.state, count: Number(row.count || 0), oldest_updated_at: iso(row.oldest_updated_at) })),
