...
function decimalToBaseUnits(value, decimals, field) {
  const raw = positiveDecimal(value, field, { maximumWholeDigits: 4, maximumFractionDigits: decimals });
...
export function createExactSolanaSpotIntent(input = {}, serverAuthority = {}) {
  ...
  if (side === "buy") {
    ...
    displayAmount = baseUnitsToDecimal(exactInput, market.native_decimals);
  } else if (side === "sell") {
  ...
  const outputMint = side === "buy" ? market.token_address : SOLANA_CANONICAL_USDC_MINT;
  ...
  return deepFreeze({
    ...
    economic_flow: side === "buy"
      ...
      : "selected_token_to_canonical_usdc",
    amount: {
      kind: amountKind,
...
function normalizeQuote(input = {}, intent) {
  ...
  const outputDecimals = intent.side === "buy" ? intent.exact_market.token_decimals : SolanaSpotQuoteReviewLimits.canonical_usdc_decimals;
  return deepFreeze({
