...
const FEE_SCHEDULES = Object.freeze({
  ...
  "0x:spot": Object.freeze({
    provider: "0x",
    chain: "evm",
    trade_type: "spot",
    fee_kind: "integrator_fee",
    fee_parameter: "swapFeeBps",
    fee_parameter_unit: "basis_points",
    maximum_fee_bps: 1_000,
    free_fee_bps: 100,
    maximum_fee_parameter_value: 1_000,
    official_limit_label: "10.00%",
    applicability: "both_swap_directions",
    venue_user_approval_required: false,
  }),
});
...
function recipientValid(schedule, recipient) {
  ...
  if (schedule.provider === "0x") return /^0x[0-9a-fA-F]{40}$/.test(recipient) && !/^0x0{40}$/i.test(recipient);
  return false;
}
