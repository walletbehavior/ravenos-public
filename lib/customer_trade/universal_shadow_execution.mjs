...
const FUNDING_SELECTIONS = new Set(["aggregate_routable_usdc", "user_chain_override", "chain_local_canonical_usdc", "chain_local_native"]);
...
export function createUniversalQuoteRequest(input = {}) {
  ...
  const requestedFundingSelection = sourceOverride
    ? "user_chain_override"
    : text(input.funding_selection || "aggregate_routable_usdc", "funding_selection", 48, { lower: true });
  if (!FUNDING_SELECTIONS.has(requestedFundingSelection)) fail("funding_selection_invalid");
  const fundingAssetInput = input.funding_asset && typeof input.funding_asset === "object" ? input.funding_asset : null;
  const fundingAsset = fundingAssetInput ? {
    chain: text(fundingAssetInput.chain, "funding_asset_chain", 32, { lower: true }),
    network: text(fundingAssetInput.network || "mainnet", "funding_asset_network", 32, { lower: true }),
    address: text(fundingAssetInput.address, "funding_asset_address", 180),
    standard: text(fundingAssetInput.standard, "funding_asset_standard", 32, { lower: true }),
    symbol: text(fundingAssetInput.symbol, "funding_asset_symbol", 40, { optional: true }) || null,
  } : null;
  return freeze({
    ...
    funding_selection: requestedFundingSelection,
...
export function createRoundTripProof({ spend_usdc, entry, exit, source_valuation = null, observed_at } = {}) {
  ...
  const observedAt = iso(observed_at, "round_trip_observed_at");
  const observedMs = Date.parse(observedAt);
  const entryCreatedMs = Date.parse(entry.created_at);
  const entryExpiresMs = Date.parse(entry.expires_at);
  if (observedMs < entryCreatedMs) fail("round_trip_timing_invalid");
  if (entryExpiresMs <= observedMs) return freeze({
    state: "entry_expired",
    observed_at: observedAt,
    expires_at: entry.expires_at,
    entry_expires_at: entry.expires_at,
    exit_expires_at: exit?.state === "route_available" ? exit.expires_at : null,
    exit_verified: false,
    trade_available: false,
    entry_quote_available: false,
    exit_quote_available: exit?.state === "route_available" && Date.parse(exit.expires_at) > observedMs,
  });
  if (!exit || exit.state !== "route_available") return freeze({
    state: "exit_unresolved",
    observed_at: observedAt,
    expires_at: entry.expires_at,
    entry_expires_at: entry.expires_at,
    exit_expires_at: null,
    exit_verified: false,
    trade_available: false,
    entry_quote_available: true,
    exit_quote_available: false,
  });
  if (entry.destination_asset_id !== exit.source_asset_id) fail("round_trip_asset_mismatch");
  const physicalRoundTrip = entry.source_asset_id === exit.destination_asset_id;
  if (!physicalRoundTrip) {
    if (!source_valuation || source_valuation.state !== "route_available") fail("round_trip_source_valuation_required");
    if (
      source_valuation.source_asset_id !== entry.source_asset_id
      || source_valuation.destination_asset_id !== exit.destination_asset_id
    ) fail("round_trip_source_valuation_asset_mismatch");
    const tolerance = Math.max(0.000001, spend * 0.000000001);
    if (Math.abs(source_valuation.expected_output - spend) > tolerance) fail("round_trip_source_valuation_amount_mismatch");
  }
  const exitCreatedMs = Date.parse(exit.created_at);
  const exitExpiresMs = Date.parse(exit.expires_at);
  if (observedMs < exitCreatedMs) fail("round_trip_timing_invalid");
  const valuationCreatedMs = physicalRoundTrip ? null : Date.parse(source_valuation.created_at);
  const valuationExpiresMs = physicalRoundTrip ? null : Date.parse(source_valuation.expires_at);
  if (!physicalRoundTrip && observedMs < valuationCreatedMs) fail("round_trip_timing_invalid");
  const expiryRows = [
    { milliseconds: entryExpiresMs, value: entry.expires_at },
    { milliseconds: exitExpiresMs, value: exit.expires_at },
    ...(!physicalRoundTrip ? [{ milliseconds: valuationExpiresMs, value: source_valuation.expires_at }] : []),
  ].sort((left, right) => left.milliseconds - right.milliseconds);
  const effectiveExpiresAt = expiryRows[0].value;
  if (exitExpiresMs <= observedMs) return freeze({
    state: "exit_expired",
    observed_at: observedAt,
    expires_at: effectiveExpiresAt,
    entry_expires_at: entry.expires_at,
    exit_expires_at: exit.expires_at,
    exit_verified: false,
    trade_available: false,
    entry_quote_available: true,
    exit_quote_available: false,
  });
  if (!physicalRoundTrip && valuationExpiresMs <= observedMs) return freeze({
    state: "source_valuation_expired",
    observed_at: observedAt,
    expires_at: effectiveExpiresAt,
    entry_expires_at: entry.expires_at,
    exit_expires_at: exit.expires_at,
    source_valuation_expires_at: source_valuation.expires_at,
    exit_verified: false,
    trade_available: false,
    entry_quote_available: true,
    exit_quote_available: true,
    source_valuation_available: false,
  });
  const entryCosts = totalKnownCosts(entry);
  const exitCosts = totalKnownCosts(exit);
  const quoteOnlyRoundTripLoss = ((spend - exit.expected_output) / spend) * 100;
  const minimumQuoteOnlyRoundTripLoss = ((spend - exit.minimum_output) / spend) * 100;
  if (entryCosts === null || exitCosts === null) return freeze({
    ...
    entry_expires_at: entry.expires_at,
    exit_expires_at: exit.expires_at,
    source_valuation_expires_at: physicalRoundTrip ? null : source_valuation.expires_at,
    source_valuation_expected_usdc: physicalRoundTrip ? null : source_valuation.expected_output,
    source_valuation_minimum_usdc: physicalRoundTrip ? null : source_valuation.minimum_output,
    physical_round_trip: physicalRoundTrip,
    economic_settlement_asset_id: exit.destination_asset_id,
    exit_verified: true,
    trade_available: false,
    entry_quote_available: true,
    exit_quote_available: true,
    ...
    quote_only_round_trip_loss_pct: quoteOnlyRoundTripLoss,
    minimum_quote_only_round_trip_loss_pct: minimumQuoteOnlyRoundTripLoss,
...
export function createUniversalShadowExecution({ request, candidates, selected, entry, exit, source_valuation = null, proof, observed_at } = {}) {
  const destination = request?.destination_asset || {};
  const refusal = proof?.exit_verified === true ? [] : [proof?.state || "route_unavailable"];
  const expired = new Set(["entry_expired", "exit_expired", "source_valuation_expired"]).has(proof?.state);
  return freeze({
    ...
    source_valuation_route: source_valuation || null,
    round_trip: proof,
    route_state: proof?.exit_verified
      ? UniversalRouteStates.EXIT_VERIFIED
      : expired
        ? UniversalRouteStates.STALE
        : entry?.state === "route_available"
          ? UniversalRouteStates.BUY_QUOTEABLE
          : UniversalRouteStates.UNAVAILABLE,
