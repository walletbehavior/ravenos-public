...
const JUPITER_INTEGRATOR_SHARE_BPS = 8_000n;
...
export function createSolanaLiveTicket(input = {}, { now = Date.now(), ttl_ms: ttlMs = 8_000 } = {}) {
  ...
  const feePolicy = input.fee_policy && typeof input.fee_policy === "object" ? input.fee_policy : {};
  if (feePolicy.enabled !== true || feePolicy.provider !== "jupiter" || feePolicy.fee_kind !== "integrator_fee") {
    fail("solana_live_fee_policy_required");
  }
  const feeBps = Number(feePolicy.fee_bps);
  if (!Number.isSafeInteger(feeBps) || feeBps < 50 || feeBps > 100) fail("solana_live_fee_bps_invalid");
  const referralAccount = publicKey(feePolicy.fee_recipient, "fee_referral_account");
  const collectorAddress = publicKey(input.fee_collector_address, "fee_collector_address");
  const feeMint = publicKey(preflight.quote?.fee_mint, "fee_mint");
  const grossFeeAmount = unsignedInteger(preflight.quote?.platform_fee_amount_base_units, "platform_fee_amount");
  const simulatedFee = preflight.simulation?.referral_fee_balance_evidence;
  if (preflight.quote?.referral_account !== referralAccount
    || Number(preflight.quote?.referral_fee_bps) !== feeBps
    || Number(preflight.quote?.fee_bps) !== feeBps
    || Number(preflight.quote?.platform_fee_bps) !== feeBps
    || grossFeeAmount <= 0n
    || simulatedFee?.independently_simulated !== true
    || simulatedFee?.referral_account !== referralAccount
    || simulatedFee?.fee_mint !== feeMint
    || Number(simulatedFee?.fee_bps) !== feeBps
    || String(simulatedFee?.gross_fee_amount_base_units) !== grossFeeAmount.toString()) {
    fail("solana_live_fee_evidence_mismatch");
  }
  const minimumCollectorCredit = (grossFeeAmount * JUPITER_INTEGRATOR_SHARE_BPS) / 10_000n;
  const simulatedCollectorCredit = unsignedInteger(simulatedFee.observed_credit_base_units, "simulated_fee_credit");
  if (String(simulatedFee.minimum_collector_credit_base_units) !== minimumCollectorCredit.toString()
    || simulatedCollectorCredit < minimumCollectorCredit
    || simulatedCollectorCredit > grossFeeAmount) {
    fail("solana_live_fee_evidence_mismatch");
  }
  ...
  const binding = Object.freeze({
    ...
    fee: Object.freeze({
      fee_bps: feeBps,
      fee_mint: feeMint,
      gross_fee_amount_base_units: grossFeeAmount.toString(),
      minimum_collector_credit_base_units: minimumCollectorCredit.toString(),
      referral_account: referralAccount,
      collector_address: collectorAddress,
      collection_method: "jupiter_referral_program",
    }),
    transaction,
  ...
  const ticket = Object.freeze({
    ...
    fee: Object.freeze({
      ...
      access_tier: clean(feePolicy.access_tier || "free", 16),
      fee_bps: feeBps,
      raven_fee_bps: feeBps,
      gross_fee_amount_base_units: grossFeeAmount.toString(),
      expected_amount_base_units: minimumCollectorCredit.toString(),
      minimum_collector_credit_base_units: minimumCollectorCredit.toString(),
      estimated_raven_fee_usdc: feeMint === SOLANA_USDC_MINT ? Number(minimumCollectorCredit) / 1_000_000 : null,
      fee_token: feeMint,
      token: feeMint,
      referral_account: referralAccount,
      recipient: referralAccount,
      collector_address: collectorAddress,
      collection_method: "jupiter_referral_program",
      collector_configured: true,
      provider_share_bps: 2_000,
...
function economicTransactionEvidence(transaction, ticket) {
  ...
  if (fee.enabled !== true
    ...
    || Number(fee.fee_bps) < 50
    || Number(fee.fee_bps) > 100) {
    fail("settled_referral_fee_policy_invalid");
  }
  const feeMint = publicKey(fee.fee_token, "settled_fee_mint");
  const referralAccount = publicKey(fee.referral_account, "settled_referral_account");
  const referralPre = tokenBalance(meta.preTokenBalances, referralAccount, feeMint);
  const referralPost = tokenBalance(meta.postTokenBalances, referralAccount, feeMint);
  if (referralPost < referralPre) fail("settled_referral_fee_direction_invalid");
  const referralCredit = referralPost - referralPre;
  const minimumCollectorCredit = unsignedInteger(fee.minimum_collector_credit_base_units, "minimum_collector_credit");
  const grossFeeAmount = unsignedInteger(fee.gross_fee_amount_base_units, "gross_fee_amount");
  if (referralCredit < minimumCollectorCredit || referralCredit > grossFeeAmount) {
    fail("settled_referral_fee_credit_mismatch");
  }
  return Object.freeze({
    ...
    raven_fee: Object.freeze({
      verified: true,
      fee_bps: Number(fee.fee_bps),
      fee_mint: feeMint,
      gross_fee_amount_base_units: grossFeeAmount.toString(),
      minimum_collector_credit_base_units: minimumCollectorCredit.toString(),
      observed_collector_credit_base_units: referralCredit.toString(),
      referral_account: referralAccount,
      collector_address: publicKey(fee.collector_address, "settled_collector_address"),
      collection_method: "jupiter_referral_program",
    }),
  });
}
  ...
  return Object.freeze({
      ...
      await db.prepare(`
        INSERT INTO ravenos_customer_live_execution_intents
          (execution_id, schema_version, user_id, venue, chain_namespace, wallet_address, exact_market_id,
           ...
           state, prepared_payload_hash, provider_request_id, expected_raven_fee_amount_base_units,
           prepared_json, expires_at, created_at, updated_at)
        VALUES (?, ?, ?, 'jupiter', 'solana', ?, ?, ?, 'market', ?, ?, ?, NULL,
                ?, ?, 'jupiter_referral_program', 'expected', 'awaiting_wallet_signature', ?, ?, ?, ?, ?, ?, ?)
      `).bind(
        ticket.ticket_id,
        ...
        ticket.fee.fee_token,
        ticket.fee.referral_account,
        ticket.binding_hash,
    ...
    async finalize({ execution_id: executionId, user_id: userId, reconciliation, now_seconds: nowSeconds }) {
      const state = new Set(["provider_confirmed", "provider_rejected", "indeterminate"]).has(reconciliation?.state)
        ...
        : "indeterminate";
      const observedFee = reconciliation?.evidence?.raven_fee?.verified === true
        ? clean(reconciliation.evidence.raven_fee.observed_collector_credit_base_units, 78)
        : null;
      const feeCollectionStatus = observedFee
        ? "observed"
        : state === "provider_rejected" ? "failed" : "indeterminate";
      await db.prepare(`
        UPDATE ravenos_customer_live_execution_intents
        SET state = ?, observed_raven_fee_amount_base_units = ?, fee_collection_status = ?, updated_at = ?
        WHERE execution_id = ? AND user_id = ? AND venue = 'jupiter' AND state IN ('submission_pending', 'reconciliation_pending')
      `).bind(state, observedFee, feeCollectionStatus, nowSeconds, executionId, userId).run();
