...
export { EVM_ZERO_X_ALLOWANCE_HOLDER } from "./evm_chain_profiles.mjs";
export const EVM_ZERO_X_CAPABILITY_SCHEMA = "ravenos.evm_zero_x_capability.v1";
export const EVM_ZERO_X_QUOTE_REQUEST_SCHEMA = "ravenos.evm_zero_x_quote_request.v1";
export const EVM_ZERO_X_UNSIGNED_QUOTE_SCHEMA = "ravenos.evm_zero_x_unsigned_quote.v1";
...
export const EvmZeroXFeeSchedule = Object.freeze({
...
export const EvmZeroXExecutionAuthorization = Object.freeze({
  quote_review: true,
...
const ACCESS_TIERS = new Set(Object.keys(EvmZeroXFeeSchedule));
...
function exactAddress(value, field, { allowNative = false } = {}) {
  ...
  if (!EVM_ADDRESS_RE.test(address) || address === ZERO_ADDRESS || (!allowNative && address === NATIVE_EVM_ASSET)) {
    fail(`${field}_invalid`);
  }
  return address;
}

function exactProfile(input, fallback = null) {
  try {
    return evmChainProfileForOrder(input, fallback);
  } catch (error) {
...
function configState(env = {}, profileSelector) {
  const profile = resolveEvmChainProfile(profileSelector);
  const prefix = profile.environment_prefix;
  const quoteRequested = enabled(env[`${prefix}_QUOTE_ENABLE`]);
  const feeRequested = enabled(env[`${prefix}_FEE_ENABLE`]);
  const endpoint = normalizeEndpointForCapability(env[`${prefix}_QUOTE_ENDPOINT`] || DEFAULT_ENDPOINT);
  const apiKeyConfigured = apiKeyValid(env.RAVENOS_ZEROX_API_KEY);
  const feeRecipient = feeRecipientOrNull(env[`${prefix}_FEE_RECIPIENT`] || env.RAVENOS_EVM_FEE_COLLECTOR_ADDRESS);
  const feeTokenSide = String(env[`${prefix}_FEE_TOKEN_SIDE`] || "sell").trim().toLowerCase();
  ...
  return {
    ...
    feeTokenSide,
    profile,
    quoteTtlMs: boundedInteger(env[`${prefix}_QUOTE_TTL_MS`], DEFAULT_QUOTE_TTL_MS, MIN_QUOTE_TTL_MS, MAX_QUOTE_TTL_MS),
    timeoutMs: boundedInteger(env[`${prefix}_TIMEOUT_MS`], DEFAULT_TIMEOUT_MS, MIN_TIMEOUT_MS, MAX_TIMEOUT_MS),
  };
}

export function resolveEvmZeroXCapability(env = {}, { profile: profileSelector } = {}) {
  const config = configState(env, profileSelector);
  const { profile } = config;
  return deepFreeze({
    schema_version: EVM_ZERO_X_CAPABILITY_SCHEMA,
    ...
    accounting_asset: profile.accounting_asset,
    ...
    fee_schedule: Object.freeze({
      free_fee_bps: EvmZeroXFeeSchedule.free,
      pro_fee_bps: EvmZeroXFeeSchedule.pro,
    ...
    execution_boundary: EvmZeroXExecutionAuthorization,
  });
}

export function createEvmZeroXQuoteRequest(input = {}, {
  ...
  const profile = exactProfile(input, profileSelector);
  const chainId = profile.chain_id;
  ...
  const configuredFeeBps = EvmZeroXFeeSchedule[tier];
  ...
  if (collectionEnabled) {
    ...
    if (side === "buy" && feeToken === NATIVE_EVM_ASSET) fail("zero_x_native_buy_token_integrator_fee_not_supported");
  ...
  const binding = {
    schema_version: EVM_ZERO_X_QUOTE_REQUEST_SCHEMA,
    provider: "0x_swap_api_v2",
...
function verifyRequestFee(request) {
  ...
  const configuredFeeBps = EvmZeroXFeeSchedule[normalizeAccessTier(request.access_tier)];
  ...
  if (side === "buy" && fee.fee_token === NATIVE_EVM_ASSET) fail("zero_x_native_buy_token_integrator_fee_not_supported");
  if (side === "sell") {
...
function verifyRequest(request, now, profileSelector = null) {
  if (request?.schema_version !== EVM_ZERO_X_QUOTE_REQUEST_SCHEMA) fail("zero_x_quote_request_required");
  const profile = exactProfile(request, profileSelector);
  if (
    request.profile_id !== profile.profile_id
    || request.chain_namespace !== profile.chain_namespace
    || request.chain_id !== profile.chain_id
    || request.canonical_chain_id !== profile.canonical_chain_id
  ) {
...
function normalizeIssues(payload, request, allowanceTarget, transactionTo) {
  ...
  const sellNative = request.sell_token === NATIVE_EVM_ASSET;
  let allowance;
  if (issues.allowance === null || issues.allowance === undefined) {
    ...
    if (
      spender !== EVM_ZERO_X_ALLOWANCE_HOLDER
      || allowanceTarget !== EVM_ZERO_X_ALLOWANCE_HOLDER
      || transactionTo !== EVM_ZERO_X_ALLOWANCE_HOLDER
...
function normalizeTransaction(payload, request, profile) {
  ...
  if (request.sell_token === NATIVE_EVM_ASSET ? value !== BigInt(request.sell_amount_base_units) : value !== 0n) {
...
function normalizeRoute(value, request, profile) {
  ...
  const routeHasAsset = (requestedAddress) => {
    if (addresses.has(requestedAddress)) return true;
    if (requestedAddress !== NATIVE_EVM_ASSET) return false;
    const wrappedNative = exactAddress(profile?.wrapped_native_token_address, "zero_x_wrapped_native_token");
    return addresses.has(wrappedNative);
  };
  // 0x represents a chain's native asset as its exact wrapped-native contract
  // inside route topology while retaining the native pseudo-address in the
  // quote binding and transaction value. Accept only the profile's verified
  // wrapped-native identity; never accept an arbitrary route alias.
  if (!routeHasAsset(request.sell_token) || !routeHasAsset(request.buy_token)) fail("zero_x_route_identity_mismatch");
...
export function normalizeEvmZeroXUnsignedQuote(payload = {}, request = {}, { profile: profileSelector, now = Date.now() } = {}) {
  const quoteTime = isoFromMillis(now, "zero_x_quote_time");
  const receivedAt = quoteTime.millis;
  const profile = verifyRequest(request, receivedAt, profileSelector);
  ...
  const transaction = normalizeTransaction(payload, request, profile);
  if (request.sell_token !== NATIVE_EVM_ASSET && allowanceTarget !== EVM_ZERO_X_ALLOWANCE_HOLDER) {
    fail("zero_x_allowance_target_untrusted");
  }
  if (
    request.sell_token !== NATIVE_EVM_ASSET
    && (transaction.to !== EVM_ZERO_X_ALLOWANCE_HOLDER || transaction.to !== allowanceTarget)
  ...
  const route = normalizeRoute(payload.route, request, profile);
  ...
  return deepFreeze({
    ok: true,
    schema_version: EVM_ZERO_X_UNSIGNED_QUOTE_SCHEMA,
    state,
    provider: "0x_swap_api_v2",
    provider_quote_id: providerQuoteId,
...
export function assertEvmZeroXQuoteFresh(quote, { profile: profileSelector, now = Date.now() } = {}) {
  if (quote?.schema_version !== EVM_ZERO_X_UNSIGNED_QUOTE_SCHEMA) fail("zero_x_unsigned_quote_required");
  ...
  const profile = exactProfile(quote, profileSelector);
  if (
  ...
  const expectedQuoteHash = digest({
    ...
    chain_namespace: quote.chain_namespace,
    chain_id: quote.chain_id,
    canonical_chain_id: quote.canonical_chain_id,
    taker: binding.taker,
...
export function createEvmZeroXQuoteClient(env = {}, {
  profile: profileSelector,
  ...
  const config = configState(env, profile);
  ...
  return Object.freeze({
    ...
    async quote(order = {}, context = {}) {
      ...
      const request = createEvmZeroXQuoteRequest(order, {
        profile,
      ...
      return normalizeEvmZeroXUnsignedQuote(payload, request, { profile, now: Number(now()) });
    },
  });
}
