...
function normalizePreflightRequest(input = {}) {
  ...
  const walletRole = text(input.wallet_role || "reference_probe", 32).toLowerCase();
  if (!["reference_probe", "canary"].includes(walletRole)) fail("wallet_role_invalid");
  ...
  if (side === "buy" && (amount < MIN_BUY_LAMPORTS || amount > MAX_BUY_LAMPORTS)) fail("canary_buy_amount_out_of_bounds");
  ...
  const inputMint = side === "buy" ? SOLANA_WRAPPED_MINT : terminal.token_address;
  const outputMint = side === "buy" ? terminal.token_address : SOLANA_WRAPPED_MINT;
...
async function fetchJupiterOrder(request, { apiKey, fetchImpl, timeoutMs, endpoint, excludedDexes }) {
  ...
  url.searchParams.set("priorityFeeLamports", MAX_PRIORITY_FEE_LAMPORTS.toString());
...
function validateOrder(order, request, nowMs, currentBlockHeight) {
  ...
  if (request.side === "sell" && outAmount > MAX_SELL_OUTPUT_LAMPORTS) fail("canary_sell_output_out_of_bounds");
  const maximumNativeDebit = request.side === "buy" ? BigInt(request.amount_base_units) + totalFee : totalFee;
  if (maximumNativeDebit > MAX_TOTAL_NATIVE_DEBIT_LAMPORTS) fail("canary_total_native_debit_out_of_bounds");
...
function nativeDeltaEvidence(preWallet, postWallet, preAccounts, postAccounts, request, quote) {
  const maximumDebit = MAX_TOTAL_NATIVE_DEBIT_LAMPORTS;
...
function safetyBlockingReasons({ review, simulation, nativeDebit }) {
  return Object.freeze([
    ...
    ...(nativeDebit > MAX_TOTAL_NATIVE_DEBIT_LAMPORTS ? ["simulated_native_debit_out_of_bounds"] : []),
  ]);
}

export async function runOperatorSolanaCanaryPreflight(input = {}, {
  rpc_url: rpcUrlInput,
  ...
  const request = normalizePreflightRequest(input);
  ...
  const requiredBalance = request.side === "buy"
  ...
  const safetyBlocking = safetyBlockingReasons({ review, simulation, nativeDebit });
  const preflightPassed = safetyBlocking.length === 0;

  const intent = Object.freeze({
    schema_version: "ravenos.operator_solana_canary_intent.v2",
  ...
  const boundaryBlocking = Object.freeze([
  ...
  return Object.freeze({
    ok: preflightPassed,
    schema_version: SOLANA_CANARY_SCHEMA,
    state: preflightPassed ? "unsigned_mainnet_preflight_passed" : "blocked",
    ...
    wallet: Object.freeze({
      ...
      low_balance_cap_lamports: MAX_CANARY_WALLET_LAMPORTS.toString(),
      low_balance_cap_satisfied: preWallet.lamports <= MAX_CANARY_WALLET_LAMPORTS,
    ...
    transaction_review: Object.freeze({
      ...
      lookup_tables: programs.lookup_tables,
      raw_transaction_returned: false,
    }),
    simulation: Object.freeze({
      ...
      maximum_native_debit_lamports: MAX_TOTAL_NATIVE_DEBIT_LAMPORTS.toString(),
    ...
    canary_readiness: Object.freeze({
      ...
      separate_low_balance_wallet_verified: request.wallet_role === "canary"
    ...
    execution_boundary: Object.freeze({
      ...
      signing_available: OperatorCanaryExecutionAuthorization.signing_for_simulation,
      browser_signing_available: false,
      customer_submission_available: false,
