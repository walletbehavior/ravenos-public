...
const MIN_INTEGRATOR_FEE_BPS = 50;
const JUPITER_INTEGRATOR_SHARE_BPS = 8_000;
...
function normalizePreflightRequest(input = {}, { customer = false } = {}) {
  ...
  const referralAccount = input.referral_account ? publicKey(input.referral_account, "referral_account") : null;
  const referralFeeBps = input.referral_fee_bps === undefined || input.referral_fee_bps === null
    ? 0
    : safeInteger(input.referral_fee_bps, "referral_fee_bps");
  if (!customer && (referralAccount || referralFeeBps)) fail("operator_referral_fee_not_allowed");
  if (customer && ((!referralAccount && referralFeeBps) || (referralAccount && !referralFeeBps))) {
    fail("referral_fee_configuration_incomplete");
  }
  if (referralFeeBps && (referralFeeBps < MIN_INTEGRATOR_FEE_BPS || referralFeeBps > MAX_FEE_BPS)) {
    fail("referral_fee_bps_out_of_bounds");
  }
  return Object.freeze({
    terminal,
...
async function fetchJupiterOrder(request, { apiKey, fetchImpl, timeoutMs, endpoint, excludedDexes }) {
  ...
  if (request.referral_account) {
    url.searchParams.set("referralAccount", request.referral_account);
    url.searchParams.set("referralFee", String(request.referral_fee_bps));
  }
...
function validateOrder(order, request, nowMs, currentBlockHeight) {
  ...
  if (order.receiver || order.maker || order.quoteId) fail("jupiter_unrequested_authority_present");
  const orderReferralAccount = order.referralAccount ? publicKey(order.referralAccount, "jupiter_referral_account") : null;
  if (request.referral_account) {
    if (orderReferralAccount !== request.referral_account) fail("jupiter_referral_account_mismatch");
  } else if (orderReferralAccount) {
    fail("jupiter_unrequested_authority_present");
  }
  ...
  const platformFeeAmount = integer(order.platformFee?.amount ?? "0", "jupiter_platform_fee_amount");
  if (request.referral_account) {
    if (feeBps !== request.referral_fee_bps || platformFeeBps !== request.referral_fee_bps) {
      fail("jupiter_referral_fee_not_applied");
    }
    if (!feeMint || platformFeeAmount <= 0n) fail("jupiter_referral_fee_evidence_missing");
  } else if (platformFeeAmount > 0n && platformFeeBps === 0) {
    fail("jupiter_platform_fee_invalid");
  }
  ...
  return Object.freeze({
    ...
    platform_fee_amount_base_units: platformFeeAmount.toString(),
    fee_mint: feeMint,
    referral_account: orderReferralAccount,
    referral_fee_bps: request.referral_fee_bps,
...
function referralFeeDeltaEvidence(preAccounts, postAccounts, request, quote) {
  if (!request.referral_account) return null;
  const feeMint = publicKey(quote.fee_mint, "jupiter_fee_mint");
  const grossFee = integer(quote.platform_fee_amount_base_units, "jupiter_platform_fee_amount");
  const pre = walletTokenBalance(preAccounts, feeMint, request.referral_account);
  const post = walletTokenBalance(postAccounts, feeMint, request.referral_account);
  if (post.amount < pre.amount) fail("simulation_referral_fee_direction_invalid");
  const credit = post.amount - pre.amount;
  const minimumCollectorCredit = (grossFee * BigInt(JUPITER_INTEGRATOR_SHARE_BPS)) / 10_000n;
  if (credit < minimumCollectorCredit || credit > grossFee) fail("simulation_referral_fee_credit_mismatch");
  return Object.freeze({
    referral_account: request.referral_account,
    fee_mint: feeMint,
    fee_bps: request.referral_fee_bps,
    gross_fee_amount_base_units: grossFee.toString(),
    minimum_collector_credit_base_units: minimumCollectorCredit.toString(),
    pre_amount_base_units: pre.amount.toString(),
    post_amount_base_units: post.amount.toString(),
    observed_credit_base_units: credit.toString(),
...
async function runSolanaPreflight(input = {}, {
  ...
  const referralFeeEvidence = referralFeeDeltaEvidence(preState.accounts, simulationRun.post_accounts, request, quote);
  ...
  const intent = Object.freeze({
    ...
    referral_account: quote.referral_account,
    referral_fee_bps: quote.referral_fee_bps,
    fee_mint: quote.fee_mint,
    platform_fee_amount_base_units: quote.platform_fee_amount_base_units,
  });
  ...
  return Object.freeze({
    ...
    simulation: Object.freeze({
      ...
      referral_fee_balance_evidence: referralFeeEvidence,
