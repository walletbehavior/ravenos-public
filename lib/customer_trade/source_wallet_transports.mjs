...
export const SourceWalletTransportLimits = Object.freeze({
  ...
  maximum_stream_watches_per_run: 25_000,
  maximum_watch_universe_size: 25_000,
...
export function normalizeSourceWalletWatchUniverse(rows = [], {
  ...
  if (rows.length > SourceWalletTransportLimits.maximum_watch_universe_size) fail("observer_watch_universe_too_large");
  const maximum = Math.max(1, Math.min(SourceWalletTransportLimits.maximum_watch_universe_size, Number(maximumWatches) || 1));
...
export function normalizeSourceWalletTransportReference(input = {}, {
  ...
  preserve_reference_received_at: preserveReferenceReceivedAt = false,
} = {}) {
  const address = normalizeSolanaWalletAddress(input.wallet_address || input.address || input?.source_wallet?.address);
  const observedAt = timestamp(
    preserveReferenceReceivedAt && input.raven_received_at ? input.raven_received_at : receivedAt,
    "observer_transport_received_at",
  );
  ...
  for (const row of references) {
    try {
      const delivery = normalizeSourceWalletTransportReference(row, {
        ...
        preserve_reference_received_at: preserveReferenceReceivedAt,
      });
