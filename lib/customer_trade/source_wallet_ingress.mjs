...
async function currentManifest(walletStore, now = new Date().toISOString(), cacheKey = walletStore) {
  ...
  if (cached && cached.expires_at > nowMs) return cached.manifest;
  ...
  manifestCache.set(usableCacheKey, { manifest, expires_at: nowMs + MANIFEST_CACHE_MS });
  return manifest;
}
