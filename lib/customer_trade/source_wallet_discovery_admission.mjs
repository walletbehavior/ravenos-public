...
export const SOURCE_WALLET_DISCOVERY_PRIORITY_SCHEMA = "ravenos.source_wallet_discovery_priority.v1";
...
export function buildSourceWalletDiscoveryResearchPriority(input = {}) {
  const observationCount = integer(input.observation_count, "wallet_discovery_priority_observation_count");
  const exactSwapShapeCount = integer(input.exact_swap_shape_count, "wallet_discovery_priority_exact_count", {
    maximum: observationCount,
  });
  const reviewedBuyInstructionCount = integer(input.reviewed_buy_instruction_count, "wallet_discovery_priority_buy_count", {
    maximum: observationCount,
  });
  if (exactSwapShapeCount + reviewedBuyInstructionCount !== observationCount) {
    fail("wallet_discovery_priority_evidence_count_mismatch");
  }
  const distinctMintCount = integer(input.distinct_mint_count, "wallet_discovery_priority_mint_count");
  const evidenceTier = candidateTier(observationCount, distinctMintCount);
  const tierPoints = evidenceTier === "high_signal" ? 200 : evidenceTier === "recurring" ? 100 : 0;
  const recurrencePoints = Math.min(200, observationCount * 20);
  const breadthPoints = Math.min(200, distinctMintCount * 40);
  const exactShapePoints = observationCount > 0
    ? Math.round((exactSwapShapeCount / observationCount) * 400)
    : 0;
  const score = Math.min(1_000, tierPoints + recurrencePoints + breadthPoints + exactShapePoints);
  return freeze({
    ...
    components: {
      ...
      breadth_points: breadthPoints,
      exact_opposing_delta_points: exactShapePoints,
    },
    ordering: [
      "research_priority_score_desc",
      "last_observed_at_desc",
      "distinct_mint_count_desc",
      "observation_count_desc",
      "candidate_id_asc",
    ],
    exact_trade_claimed: false,
...
function normalizeCandidateRow(row) {
  ...
  const researchPriority = buildSourceWalletDiscoveryResearchPriority({
    observation_count: observationCount,
    exact_swap_shape_count: row.exact_swap_shape_count,
    reviewed_buy_instruction_count: row.reviewed_buy_instruction_count,
    distinct_mint_count: distinctMintCount,
  });
  if (integer(row.research_priority_version, "wallet_discovery_priority_version", { minimum: 1 }) !== researchPriority.score_version
    || integer(row.research_priority_score, "wallet_discovery_priority_score", { maximum: 1_000 }) !== researchPriority.score) {
    fail("wallet_discovery_candidate_priority_mismatch");
  }
  return freeze({
  ...
  return freeze({
    ...
    async ingestBatch(batchInput, { body_sha256: bodySha256, key_id: keyId, now = Date.now() } = {}) {
      ...
      for (const inputObservation of batch.observations) {
        ...
        await db.prepare(`
          INSERT INTO ravenos_source_wallet_discovery_candidates (
            ...
            distinct_mint_count, research_priority_score, research_priority_version,
            first_observed_at, last_observed_at, latest_observation_id,
            ...
            0, 0, 0, 0, 0, 1, ?, ?, ?, ?, ?, 0, ?, NULL, NULL, NULL, NULL, NULL, ?, ?)
      ...
      for (const candidateId of affected) {
        ...
        const exactSwapShapeCount = Number(counts?.exact_count || 0);
        const reviewedBuyInstructionCount = Number(counts?.buy_count || 0);
        const researchPriority = buildSourceWalletDiscoveryResearchPriority({
          observation_count: observationCount,
          exact_swap_shape_count: exactSwapShapeCount,
          reviewed_buy_instruction_count: reviewedBuyInstructionCount,
          distinct_mint_count: distinctMintCount,
        });
        ...
        await db.prepare(`
          UPDATE ravenos_source_wallet_discovery_candidates SET
            ...
            research_priority_score = ?, research_priority_version = ?,
        ...
        `).bind(
          ...
          observationCount,
          exactSwapShapeCount,
          reviewedBuyInstructionCount,
          distinctMintCount,
          researchPriority.score,
          researchPriority.score_version,
          seconds,
          seconds,
          candidateId,
        ).run();
    ...
    async leaseCandidates({ worker_id: workerId, now = Date.now(), limit = SourceWalletDiscoveryAdmissionLimits.maximum_jobs_per_run } = {}) {
      ...
      const rows = await db.prepare(`
        ...
        ORDER BY research_priority_score DESC, last_observed_at DESC,
          distinct_mint_count DESC, observation_count DESC, candidate_id ASC
        LIMIT ?
    ...
    async health({ now = Date.now() } = {}) {
      ...
      const totals = await db.prepare(`
        SELECT COUNT(*) AS candidate_count, COALESCE(SUM(observation_count), 0) AS observation_count,
          ...
          SUM(CASE WHEN research_priority_score >= 800 THEN 1 ELSE 0 END) AS priority_800_plus_count,
          SUM(CASE WHEN research_priority_score >= 600 AND research_priority_score < 800 THEN 1 ELSE 0 END) AS priority_600_799_count,
          SUM(CASE WHEN research_priority_score >= 400 AND research_priority_score < 600 THEN 1 ELSE 0 END) AS priority_400_599_count,
          SUM(CASE WHEN research_priority_score < 400 THEN 1 ELSE 0 END) AS priority_below_400_count
      ...
      return freeze({
        ...
        research_priority: {
          score_version: 1,
          priority_800_plus_count: Number(totals?.priority_800_plus_count || 0),
          priority_600_799_count: Number(totals?.priority_600_799_count || 0),
          priority_400_599_count: Number(totals?.priority_400_599_count || 0),
          priority_below_400_count: Number(totals?.priority_below_400_count || 0),
          subscriber_identity_included: false,
        },
