...
export function normalizeWalletScreenerRequest(input = {}, { now, copyability_reference: copyabilityReference = null } = {}) {
  ...
  if (!WalletScreenerChains.includes(chain)) fail("wallet_screener_chain_invalid");
  ...
  return freeze({
    ...
    scope: `raven_indexed_${chain}_wallets`,
    chain,
    network,
...
export function buildWalletScreenerResponse({ query, rows, total, now } = {}) {
  ...
  return freeze({
    ...
    scope: {
      universe: `raven_indexed_${normalized.chain}_wallets`,
