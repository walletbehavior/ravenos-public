...
export const WalletScreenerSorts = Object.freeze([
  ...
  "follower_route_persistence_desc",
  "follower_return_desc",
  "follower_capture_desc",
...
export const WalletScreenerChains = Object.freeze(["solana", "robinhood"]);
export const WalletScreenerChainScopes = Object.freeze(["all", ...WalletScreenerChains]);
...
export const WalletScreenerFieldSqlColumns = Object.freeze({
  ...
  outcome_checkpoint_count: "cp.outcome_checkpoint_count",
  follower_route_persistence_pct: "cp.follower_route_persistence_pct",
  median_follower_return_pct: "cp.median_follower_return_pct",
  follower_win_rate_pct: "cp.follower_win_rate_pct",
  follower_capture_sample_count: "cp.follower_capture_sample_count",
  follower_capture_ratio_pct: "cp.follower_capture_ratio_pct",
  follower_minus_source_return_pct: "cp.follower_minus_source_return_pct",
...
const NUMERIC_FIELDS = Object.freeze({
  ...
  outcome_checkpoint_count: [0, 10_000_000],
  follower_route_persistence_pct: [0, 100],
  median_follower_return_pct: [-1_000_000_000, 1_000_000_000],
  follower_win_rate_pct: [0, 100],
  follower_capture_sample_count: [0, 10_000_000],
  follower_capture_ratio_pct: [-1_000_000_000, 1_000_000_000],
  follower_minus_source_return_pct: [-1_000_000_000, 1_000_000_000],
...
export const WalletScreenerPresets = Object.freeze({
  ...
  follower_tested: Object.freeze({
    ...
    summary: "Prospective $100 entry, reverse-exit, and +1h follower outcomes with persistent routes.",
    clauses: Object.freeze([
      ...
      Object.freeze({ field: "outcome_checkpoint_count", operator: "gte", value: 20 }),
      Object.freeze({ field: "follower_route_persistence_pct", operator: "gte", value: 75 }),
    ]),
  }),
});
...
const REQUEST_KEYS = new Set(["chain", "network", "filters", "clauses", "preset", "sort", "page", "page_size"]);
...
export function normalizeWalletScreenerRequest(input = {}, { now, copyability_reference: copyabilityReference = null } = {}) {
  ...
  const chain = String(request.chain || "solana").trim().toLowerCase();
  if (!WalletScreenerChainScopes.includes(chain)) fail("wallet_screener_chain_invalid");
  const network = String(request.network || "mainnet").trim().toLowerCase();
  if (network !== "mainnet") fail("wallet_screener_network_invalid");
  ...
  return freeze({
    ...
    scope: chain === "all" ? "raven_indexed_supported_wallets" : `raven_indexed_${chain}_wallets`,
    chain,
    network,
...
function whySurfaced({ chain, lastTradeAt, tradeCount, activeDays, knownCostBasisPct, closedLots, reconstructionConfidencePct, topOneConcentrationPct, copyabilitySampleCount, policyPassPct }) {
  const reasons = [];
  ...
  if (!reasons.length) reasons.push({
    ...
    label: `An exact ${chain === "robinhood" ? "Robinhood Chain" : "Solana"} wallet record exists in Raven's bounded index.`,
    value: null,
    unit: null,
  });
...
export function projectWalletScreenerRow(row) {
  ...
  const chain = String(row.chain || "solana").trim().toLowerCase();
  if (!WalletScreenerChains.includes(chain)) return null;
  const network = String(row.network || "mainnet").trim().toLowerCase();
  const sourceWalletId = projectedIdentifier(row.source_wallet_id, /^sw_(?:sol|rh)_[a-f0-9]{40}$/);
  ...
  try {
    identity = normalizeSourceWalletChainIdentity({
      chain,
      network,
      ...(chain === "robinhood" ? { chain_id: Number(row.chain_id || 4663) } : {}),
      address: row.address,
    });
  } catch {
    return null;
  }
  if (identity.source_wallet_id !== sourceWalletId) return null;
  ...
  const outcomeCheckpointCount = optionalProjectedInteger(row.outcome_checkpoint_count, { minimum: 0, maximum: 10_000_000 });
  const outcomeReferenceHorizonSeconds = optionalProjectedInteger(row.outcome_reference_horizon_seconds, { minimum: 1, maximum: 86_400 });
  const followerRoutePersistencePct = optionalProjectedNumber(row.follower_route_persistence_pct, { minimum: 0, maximum: 100 });
  const medianFollowerReturnPct = optionalProjectedNumber(row.median_follower_return_pct);
  const followerWinRatePct = optionalProjectedNumber(row.follower_win_rate_pct, { minimum: 0, maximum: 100 });
  const followerCaptureSampleCount = optionalProjectedInteger(row.follower_capture_sample_count, { minimum: 0, maximum: 10_000_000 });
  const followerCaptureRatioPct = optionalProjectedNumber(row.follower_capture_ratio_pct);
  const followerMinusSourceReturnPct = optionalProjectedNumber(row.follower_minus_source_return_pct);
  ...
  const followerReality = {
    ...
    outcome_checkpoint_count: outcomeCheckpointCount,
    outcome_reference_horizon_seconds: outcomeReferenceHorizonSeconds,
    route_persistence_pct: followerRoutePersistencePct,
    median_follower_return_pct: medianFollowerReturnPct,
    follower_win_rate_pct: followerWinRatePct,
    follower_capture_sample_count: followerCaptureSampleCount,
    follower_capture_ratio_pct: followerCaptureRatioPct,
    follower_minus_source_return_pct: followerMinusSourceReturnPct,
  ...
  return freeze({
    ...
    why_surfaced: whySurfaced({ chain, lastTradeAt, tradeCount, activeDays, knownCostBasisPct, closedLots, reconstructionConfidencePct, topOneConcentrationPct, copyabilitySampleCount, policyPassPct }),
    follower_reality: followerReality,
    ...
    scope: {
      universe: `raven_indexed_${chain}_wallets`,
...
export function buildWalletScreenerResponse({ query, rows, total, now } = {}) {
  ...
  return freeze({
    ...
    scope: {
      universe: normalized.chain === "all" ? "raven_indexed_supported_wallets" : `raven_indexed_${normalized.chain}_wallets`,
      chain: normalized.chain,
      chains: normalized.chain === "all" ? [...WalletScreenerChains] : [normalized.chain],
    ...
    limitations: [
      normalized.chain === "all"
        ? "This combines Raven's bounded Solana and Robinhood Chain indexes; it is not every wallet on either chain."
        : `This is Raven's bounded indexed ${normalized.chain === "robinhood" ? "Robinhood Chain" : "Solana"} wallet universe, not every wallet on the chain.`,
