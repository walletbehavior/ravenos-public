...
export function subscriptionConfig(env = {}) {
  ...
  return {
    ...
    atlasMonthlyPriceId: env.STRIPE_ATLAS_MONTHLY_PRICE_ID || env.RAVENOS_STRIPE_ATLAS_MONTHLY_PRICE_ID || "",
    atlasYearlyPriceId: env.STRIPE_ATLAS_YEARLY_PRICE_ID || env.STRIPE_ATLAS_ANNUAL_PRICE_ID || env.RAVENOS_STRIPE_ATLAS_YEARLY_PRICE_ID || "",
...
export function planTypeForPriceId(priceId, env = {}) {
  ...
  if (priceId && priceId === config.atlasMonthlyPriceId) return "atlas_monthly";
  if (priceId && priceId === config.atlasYearlyPriceId) return "atlas_annual";
