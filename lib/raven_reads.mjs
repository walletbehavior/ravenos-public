...
function technicalReadCopy(type, metadata = {}, freshnessState = "unknown") {
  ...
  if (type === "technical-macd-crossover") {
    ...
    return {
      ...
      plainEnglish: `MACD crossed ${positive ? "above" : "below"} its signal line on a closed candle. This describes momentum, not direction certainty.`,
  ...
  if (type === "technical-accumulation-zone") {
    return {
      ...
      plainEnglish: "Price compressed while closed-candle volume and range position stayed constructive. This is not proof of wallet accumulation.",
  ...
  return {
    ...
    plainEnglish: `The ${ratioLabel} level is measured between two confirmed swing pivots. It is a reference, not predicted support or resistance.`,
...
export function translateOverlayToRavenRead(overlay, context = {}) {
  ...
  if (["technical-macd-crossover", "technical-accumulation-zone", "technical-fibonacci-level"].includes(type)) {
    ...
    return baseRead({
      ...
      evidenceRole: copy.evidenceRole,
