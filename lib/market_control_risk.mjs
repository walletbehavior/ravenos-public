...
export function buildMarketControlRiskProjection({
  ...
  const holderSource = clean(holders?.source?.label, 80) || "On-chain holder index";

  const top3WalletPct = finite(holders?.summary?.top_3_wallet_supply_pct, 0, 100);
  const top10WalletPct = finite(holders?.summary?.top_10_wallet_supply_pct, 0, 100);
  const largestWalletPct = finite(holders?.summary?.largest_non_pool_wallet_supply_pct, 0, 100) ?? (Array.isArray(holders?.holders) ? holders.holders : [])
    .filter((row) => row?.excluded_from_wallet_concentration !== true)
    .map((row) => finite(row?.supply_share_pct, 0, 100))
    .filter((value) => value !== null)
    .sort((left, right) => right - left)[0] ?? null;
  if (top3WalletPct !== null) {
    facts.push(evidence({
      id: "top_3_wallet_concentration",
      label: "Top 3 wallets",
      detail: `${top3WalletPct.toFixed(1)}% of supply after excluding the exact pool account.`,
      severity: "info",
      dimension: "control",
      source: holderSource,
      observedAt: holderObservedAt,
    }));
    if (top3WalletPct >= 40) risks.push(evidence({ id: "top_3_wallet_concentration_high", label: "Top 3 wallets dominate supply", detail: `The top 3 non-pool wallets hold ${top3WalletPct.toFixed(1)}% of supply.`, severity: "high", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
    else if (top3WalletPct >= 25) risks.push(evidence({ id: "top_3_wallet_concentration_elevated", label: "Top 3 wallet concentration", detail: `The top 3 non-pool wallets hold ${top3WalletPct.toFixed(1)}% of supply.`, severity: "elevated", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
  }
  if (top10WalletPct !== null) {
    ...
    if (top10WalletPct >= 50) risks.push(evidence({ id: "top_10_wallet_concentration_high", label: "Highly concentrated holders", detail: `The top 10 non-pool wallets hold ${top10WalletPct.toFixed(1)}% of supply.`, severity: "high", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
    else if (top10WalletPct >= 30) risks.push(evidence({ id: "top_10_wallet_concentration_elevated", label: "Concentrated holders", detail: `The top 10 non-pool wallets hold ${top10WalletPct.toFixed(1)}% of supply.`, severity: "elevated", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
    else mitigants.push(evidence({ id: "top_10_wallet_concentration_bounded", label: "Pool-excluded concentration", detail: `Top 10 non-pool wallets hold ${top10WalletPct.toFixed(1)}% of supply.`, severity: "positive", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
  } else unknowns.push("Top-wallet concentration after excluding the exact pool");

  if (largestWalletPct !== null) {
    if (largestWalletPct >= 15) risks.push(evidence({ id: "largest_wallet_high", label: "Large single holder", detail: `The largest observed non-pool wallet holds ${largestWalletPct.toFixed(1)}% of supply.`, severity: "high", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
    else if (largestWalletPct >= 8) risks.push(evidence({ id: "largest_wallet_elevated", label: "Notable single holder", detail: `The largest observed non-pool wallet holds ${largestWalletPct.toFixed(1)}% of supply.`, severity: "elevated", dimension: "control", source: holderSource, observedAt: holderObservedAt }));
  }
  ...
  const holderContextParts = [];
  if (top3WalletPct !== null) holderContextParts.push(`top-3 is ${top3WalletPct.toFixed(1)}%`);
  if (top10WalletPct !== null) holderContextParts.push(`top-10 is ${top10WalletPct.toFixed(1)}%`);
  const holderContext = holderContextParts.length ? ` Pool-excluded wallet concentration: ${holderContextParts.join("; ")}.` : "";
  const holderDistributionState = top3WalletPct === null && top10WalletPct === null && largestWalletPct === null
    ? "unavailable"
    : top3WalletPct >= 40 || top10WalletPct >= 50 || largestWalletPct >= 15
      ? "highly_concentrated"
      : top3WalletPct >= 25 || top10WalletPct >= 30 || largestWalletPct >= 8
        ? "concentrated"
        : "more_distributed";
  ...
  return Object.freeze({
    ...
    metrics: Object.freeze({
      top_3_wallet_supply_pct: top3WalletPct,
    ...
    interpretation: Object.freeze({
      ...
      holder_distribution_state: holderDistributionState,
