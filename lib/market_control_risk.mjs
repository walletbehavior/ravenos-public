...
export function buildMarketControlRiskProjection({
  ...
  if (top3WalletPct !== null) {
    ...
    if (top3WalletPct >= 40) risks.push(evidence({ id: "top_3_wallet_concentration_high", label: "Top 3 wallets dominate supply", detail: `The top 3 non-pool wallets hold ${top3WalletPct.toFixed(1)}% of supply.`, severity: "high", dimension: "control", source: "Solana on-chain accounts", observedAt: holderObservedAt }));
    else if (top3WalletPct >= 25) risks.push(evidence({ id: "top_3_wallet_concentration_elevated", label: "Top 3 wallet concentration", detail: `The top 3 non-pool wallets hold ${top3WalletPct.toFixed(1)}% of supply.`, severity: "elevated", dimension: "control", source: "Solana on-chain accounts", observedAt: holderObservedAt }));
  }
