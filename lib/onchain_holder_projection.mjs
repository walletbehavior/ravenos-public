...
async function buildLargestAccountFallback({ rpc, identity, supplyResult, observedAt, sourceLabel, indexedState = "unavailable" }) {
...
async function buildHolderSnapshot({ runtime, identity, fetchImpl, observedAt }) {
  ...
  try {
    ...
    if (!census.complete) {
      return buildLargestAccountFallback({ rpc, identity, supplyResult, observedAt, sourceLabel: runtime.source_label, indexedState: "bounded_before_completion" });
    }
    ...
    return buildLargestAccountFallback({ rpc, identity, supplyResult, observedAt, sourceLabel: runtime.source_label, indexedState: "unavailable" });
  }
}
