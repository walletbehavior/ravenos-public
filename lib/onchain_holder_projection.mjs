...
const INDEXED_TOKEN_ACCOUNT_PAGE_SIZE = 1_000;
...
function parsedIndexedTokenAccount(row, expectedMint, expectedMintBytes, tokenProgram) {
  const nested = parsedProgramTokenAccount(row, expectedMintBytes, tokenProgram);
  if (nested) return nested;
  const tokenAccountAddress = clean(row?.address, 64);
  const mint = clean(row?.mint, 64);
  const holderAddress = clean(row?.owner, 64);
  const rawAmount = row?.amount;
  const amount = typeof rawAmount === "number"
    ? Number.isSafeInteger(rawAmount) && rawAmount >= 0 ? String(rawAmount) : null
    : integerString(rawAmount);
  if (
    !SOLANA_ADDRESS_RE.test(tokenAccountAddress)
    || mint !== expectedMint
    || !SOLANA_ADDRESS_RE.test(holderAddress)
    || amount === null
  ) return null;
  return Object.freeze({
    holder_address: holderAddress,
...
async function readIndexedTokenAccountCensus({ rpc, identity, mintAccount, supply }) {
  ...
  while (pageCount < MAX_CENSUS_PAGES && accountsByAddress.size < MAX_CENSUS_SOURCE_ACCOUNTS) {
    const params = {
      mintAddress: identity.token_address,
      limit: INDEXED_TOKEN_ACCOUNT_PAGE_SIZE,
      options: { showZeroBalance: false },
    };
    if (cursor) params.cursor = cursor;
    const result = await rpc("getTokenAccounts", params);
    const values = Array.isArray(result?.token_accounts) ? result.token_accounts : null;
    if (!values || values.length > INDEXED_TOKEN_ACCOUNT_PAGE_SIZE) fail("holder_rpc_response_invalid");
    const slot = safeInteger(result?.last_indexed_slot);
    if (slot !== null && slot > 0) slots.push(slot);
    for (const row of values) {
      if (accountsByAddress.size >= MAX_CENSUS_SOURCE_ACCOUNTS) break;
      const parsed = parsedIndexedTokenAccount(row, identity.token_address, expectedMintBytes, tokenProgram);
    ...
    const nextCursor = clean(result?.cursor, 512) || null;
    if (!nextCursor) {
      complete = total !== null
        ? accountsByAddress.size >= total
        : values.length < INDEXED_TOKEN_ACCOUNT_PAGE_SIZE;
      break;
    }
    if (cursors.has(nextCursor)) fail("holder_rpc_pagination_invalid");
  ...
  return Object.freeze({
    ...
    source_method: "indexed_token_accounts",
    supply,
  });
}
...
async function readCompleteProgramAccountCensus({ rpc, identity, mintAccount, supply }) {
  ...
  return Object.freeze({
    ...
    slot_min: slots.length ? Math.min(...slots) : null,
    slot_max: slots.length ? Math.max(...slots) : null,
    source_method: "indexed_program_account_scan",
    supply,
  });
}
...
async function buildHolderSnapshot({ runtime, identity, fetchImpl, observedAt }) {
  ...
  try {
    ...
    let census;
    try {
      census = await readIndexedTokenAccountCensus({ rpc, identity, mintAccount, supply });
    } catch (error) {
      if (error?.status === 400 || error?.status === 409) throw error;
      census = await readCompleteProgramAccountCensus({ rpc, identity, mintAccount, supply });
    }
    if (!census.complete) {
    ...
    return Object.freeze({
      ...
      holders,
      source: Object.freeze({ label: runtime.source_label, network: "mainnet-beta", method: census.source_method, raw_rpc_included: false }),
      limitations: Object.freeze([
...
export const OnchainHolderProjectionContract = Object.freeze({
  ...
  indexed_token_account_page_size: INDEXED_TOKEN_ACCOUNT_PAGE_SIZE,
