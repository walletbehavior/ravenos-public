...
function publicActivityAsset(asset) {
  ...
  return Object.freeze({
    ...
    standard: clean(asset.standard, 40) || null,
...
function publicSourceWalletActivityEvent(event, expectedSourceId) {
  if (event?.schema_version !== SOLANA_WALLET_EVENT_SCHEMA) throw new CustomerWalletCopyError("stored_wallet_activity_event_invalid");
  if (!/^swe_[a-f0-9]{40}$/.test(String(event.event_id || ""))) throw new CustomerWalletCopyError("stored_wallet_activity_event_invalid");
  let address;
  try {
    address = normalizeSolanaWalletAddress(event.source_wallet?.address);
  } catch {
    throw new CustomerWalletCopyError("stored_wallet_activity_event_invalid");
  }
  if (sourceWalletId(address) !== expectedSourceId || !SolanaWalletEventKinds.includes(event.classification?.kind)) {
    throw new CustomerWalletCopyError("stored_wallet_activity_event_invalid");
  }
  const signature = clean(event.chain_evidence?.signature, 100);
  if (signature.length < 64) throw new CustomerWalletCopyError("stored_wallet_activity_event_invalid");
  ...
  return Object.freeze({
    ...
    source_wallet: Object.freeze({ chain: "solana", network: "mainnet", address }),
    chain_evidence: Object.freeze({
      signature,
      slot: Number.isSafeInteger(Number(event.chain_evidence?.slot)) ? Number(event.chain_evidence.slot) : null,
      block_time: event.chain_evidence?.block_time || null,
      finality: event.chain_evidence?.finality || null,
      provider: event.chain_evidence?.provider || null,
      evidence_reference: `solana:signature:${signature}`,
    }),
    timing: Object.freeze({
      observation_mode: event.timing?.observation_mode || null,
      raven_received_at: event.timing?.raven_received_at || null,
    ...
    economic: Object.freeze({
      source_asset: publicActivityAsset(event.economic?.source_asset),
      destination_asset: publicActivityAsset(event.economic?.destination_asset),
...
export function resolveWalletCopyActivation(env = {}) {
  ...
  return Object.freeze({
    ...
    scheduler: observer.evaluator || backfill.evaluator,
...
export function createD1CustomerWalletCopyStore(db) {
  ...
  return Object.freeze({
    ...
    async recordEvents(sourceId, events, now) {
      const inserted = [];
      const eventStatement = (event) => db.prepare(`
      ...
      const finalityStatement = (event) => db.prepare(`
    ...
    async listSourceEventPage(sourceId, { kinds = null, limit = 12, cursor = null } = {}) {
      ...
      if (cursor) {
        conditions.push("(COALESCE(block_time, observed_at) < ? OR (COALESCE(block_time, observed_at) = ? AND event_id < ?))");
        bindings.push(cursor.order_time, cursor.order_time, cursor.event_id);
      }
      const where = conditions.join(" AND ");
      const result = await db.prepare(`
        SELECT event_id, event_json, COALESCE(block_time, observed_at) AS order_time
      ...
      return Object.freeze({
        events: visible.map((row) => parseJson(row.event_json)).filter((row) => row?.schema_version === SOLANA_WALLET_EVENT_SCHEMA),
...
function routeMatch(pathname) {
  ...
  const sourceWalletEvents = pathname.match(/^\/api\/v1\/wallet-copy\/wallets\/(sw_sol_[a-f0-9]{40})\/events$/);
...
async function prospectiveCopyabilityForSource(store, sourceId, now, referenceFeeBps = null) {
  ...
  return buildSourceWalletCopyabilityMatrix(observations, { generated_at: generatedAt, reference_fee_bps: referenceFeeBps });
}

async function inspectAddress({ address, store, provider, now, reference_fee_bps: referenceFeeBps = null }) {
  ...
  const prospectiveCopyability = await prospectiveCopyabilityForSource(store, sourceId, now, referenceFeeBps);
...
async function enqueueDeepHistory(backfillStore, { address, now }) {
...
export async function routeCustomerWalletCopy(request, env = {}, deps = {}) {
  ...
  if (!sameOriginBoundary(request) || url.hash || (url.search && route.kind !== "source_wallet_events")) return privateJson({ ok: false, schema_version: CUSTOMER_WALLET_COPY_SCHEMA, error: "request_not_allowed" }, { status: 403 });
  ...
  if (new Set(["screener", "source_wallet", "source_wallet_events", "research_saves", "research_save"]).has(route.kind) && !activation.wallet_screener) {
  ...
  const backfillStore = activation.deep_history ? deps.sourceWalletBackfillStore || null : null;
  try {
    ...
    if (route.kind === "inspect") {
      ...
      const deepHistory = await enqueueDeepHistory(backfillStore, { address, now });
      ...
      return privateJson({ ok: true, schema_version: CUSTOMER_WALLET_COPY_SCHEMA, state: "available", ...result, recent_events: activity.events, activity, deep_history: deepHistory }, {}, authorized.authorization);
    }

    if (route.kind === "screener") {
      const query = normalizeWalletScreenerRequest(await parseBody(request), {
        now,
        copyability_reference: createSourceWalletCopyabilityPolicyReference({
    ...
    if (route.kind === "research_saves" && request.method === "POST") {
      ...
      const deepHistory = await enqueueDeepHistory(backfillStore, { address: source.address, now });
    ...
    if (route.kind === "source_wallet") {
      ...
      const deepHistory = await deepHistoryForSource(backfillStore, route.source_wallet_id);
      const prospectiveCopyability = await prospectiveCopyabilityForSource(
        store,
      ...
      return privateJson({
        ...
        profile,
        prospective_copyability: prospectiveCopyability,
