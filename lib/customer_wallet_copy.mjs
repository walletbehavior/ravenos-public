...
function profileSnapshotId(sourceId, profile) {
  return `swp_${digest([sourceId, profile.generated_at, JSON.stringify(profile.coverage)])}`;
}
...
export function resolveWalletCopyActivation(env = {}) {
  ...
  return Object.freeze({
    ...
    continuous_observer: false,
    scheduler: false,
    monitoring_mode: "manual_refresh",
  });
}
...
function responseError(error, authorization = null) {
  const code = error instanceof CustomerWalletCopyError ? error.code : "wallet_copy_state_unavailable";
  const status = code === "wallet_copy_request_too_large" ? 413
    : new Set(["wallet_copy_watch_not_found"]).has(code) ? 404
      : new Set(["wallet_copy_watch_quota_exceeded", "wallet_copy_watch_revision_conflict", "source_cursor_gap"]).has(code) ? 409
        : new Set(["wallet_copy_provider_unavailable", "wallet_copy_state_unavailable", "stored_wallet_copy_state_invalid"]).has(code) ? 503
          : 400;
...
async function persistProfile(store, sourceId, now) {
  const allEvents = await store.listSourceEvents(sourceId, 500);
  if (!allEvents.length) return null;
  const profile = buildSolanaWalletProfile(allEvents, { generated_at: new Date(now * 1_000).toISOString() });
...
async function inspectAddress({ address, store, provider, now }) {
  ...
  const profile = await persistProfile(store, sourceId, now);
...
async function evaluateNewSignals({ events, watch, store, provider, userId, now }) {
  ...
  for (const event of eligible) {
    ...
    try {
      evidence = typeof provider.quoteCopySignal === "function"
        ? await provider.quoteCopySignal({ event, policy, now })
    ...
    await store.recordDecision(userId, decision, now);
    if (decision.decision.state === "SHADOW_EXECUTABLE") {
      await store.recordPosition(userId, createShadowCopyPosition(decision), now);
    }
...
export async function routeCustomerWalletCopy(request, env = {}, deps = {}) {
  ...
  try {
    const limited = await applyRateLimit(authorized.authorization, request, env, deps, route, mutation);
    ...
    if (route.kind === "refresh") {
      ...
      const profile = await persistProfile(store, row.source_wallet_id, now);
      return privateJson({
        ok: true,
        schema_version: CUSTOMER_WALLET_COPY_SCHEMA,
    ...
    if (route.kind === "decisions") {
      ...
      return privateJson({
        ...
        decisions,
        copyability: [...byWatch.entries()].map(([watchIdentifier, rows]) => ({ watch_id: watchIdentifier, snapshot: buildCopyabilitySnapshot(rows, { generated_at: new Date(now * 1_000).toISOString() }) })),
      }, {}, authorized.authorization);
    }
...
export const CustomerWalletCopyContract = Object.freeze({
  ...
  source_level_disabled: Object.freeze({ live_copy: true, signing: true, broadcasting: true, custody: true, fee_collection: true, scheduler: true }),
});
