...
async function persistProfile(store, sourceId, now, history = null) {
...
async function inspectAddress({ address, store, provider, now }) {
  ...
  const profile = await persistProfile(store, sourceId, now, loaded);
...
export async function routeCustomerWalletCopy(request, env = {}, deps = {}) {
  ...
  try {
    ...
    if (route.kind === "refresh") {
      ...
      const profile = await persistProfile(store, row.source_wallet_id, now, initial ? loaded : null);
      return privateJson({
        ok: true,
        schema_version: CUSTOMER_WALLET_COPY_SCHEMA,
