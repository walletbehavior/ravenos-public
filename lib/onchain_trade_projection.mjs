...
const EXPLORERS = Object.freeze({
  ...
  robinhood: Object.freeze({ transaction: "https://robinhoodchain.blockscout.com/tx/", account: "https://robinhoodchain.blockscout.com/address/" }),
});
...
export const OnchainTradeProjectionContract = Object.freeze({
  ...
  evm_pool_identity: "20_byte_address_or_32_byte_pool_id",
  evm_token_and_actor_identity: "20_byte_address_only",
...
function cleanPoolAddress(chain, value) {
  const address = String(value || "").trim();
  if (chain === "solana") return SOLANA_ADDRESS_RE.test(address) ? address : null;
  // Concentrated-liquidity EVM pools are ordinarily contracts, but v4 pools
  // can be identified by a bytes32 pool ID. This exception is deliberately
  // limited to the pool field; token, quote, and actor identities remain
  // strict 20-byte EVM addresses.
  return EVM_POOL_ID_RE.test(address) ? address.toLowerCase() : null;
}
...
function normalizeIdentity(identity = {}) {
  ...
  const poolAddress = cleanPoolAddress(chain, identity.pool_address);
