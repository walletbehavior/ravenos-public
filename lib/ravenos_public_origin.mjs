...
const PUBLIC_PROJECTION_FETCH_REVISION = "2";
...
// The protected origin shares a bounded Raven host with scheduled research
// jobs. Normal responses are fast, but short CPU-contention spikes can exceed
// three seconds. Keep this finite and below the Worker request budget while
// allowing a current, already-public projection to finish loading.
const DEFAULT_TIMEOUT_MS = 8_000;
const DEFAULT_ATLAS_UNIVERSE_TIMEOUT_MS = 3_000;
const DEFAULT_ATLAS_HYDRATION_TIMEOUT_MS = 8_000;
const ATLAS_HYDRATION_ENDPOINTS = new Set([
  "history",
  "options_expirations",
  "options_chain",
  "sec_filings",
...
export function atlasUniverseTimeoutMs(env = {}, endpoint = "") {
  const fallback = ATLAS_HYDRATION_ENDPOINTS.has(String(endpoint || ""))
    ? DEFAULT_ATLAS_HYDRATION_TIMEOUT_MS
    : DEFAULT_ATLAS_UNIVERSE_TIMEOUT_MS;
  return finitePositive(env.RAVENOS_PUBLIC_ORIGIN_ATLAS_TIMEOUT_MS, fallback);
}
...
function originRequest(env, key) {
  ...
  const url = new URL(`${base}/${encodeURIComponent(key)}.json`);
  // The protected origin now serves atomically replaced documents without
  // caching. This revision moves every projection off any older edge object
  // created before that policy was active.
  url.searchParams.set("projection", PUBLIC_PROJECTION_FETCH_REVISION);
  return {
    ok: true,
    url: url.toString(),
    headers: {
...
function sanitizeAtlasProviderView(view, { entityId = "" } = {}) {
  if (!view || typeof view !== "object") return null;
  const upstreamPolicy = view.display_policy && typeof view.display_policy === "object" ? {
  ...
  const decision = atlasObservationDecision(view.provider, upstreamPolicy, { entityId });
  const observationAllowed = decision.decision === "allowed";
  const policy = upstreamPolicy ? {
    ...upstreamPolicy,
    decision: decision.decision,
    raw_redistribution_allowed: observationAllowed,
    cache_allowed: observationAllowed && upstreamPolicy.cache_allowed === true,
    reason: decision.reason || upstreamPolicy.reason,
    decision_source: decision.source === "qualified_upstream_policy" ? upstreamPolicy.decision_source : decision.source,
  } : {
    decision: decision.decision,
  ...
  const refusalReasons = Array.isArray(view.refusal_reasons) ? view.refusal_reasons.slice(0, 20).map((reason) => String(reason).slice(0, 240)) : [];
  if (!observationAllowed && decision.reason && !refusalReasons.includes(decision.reason)) refusalReasons.push(decision.reason);
  return {
    state: observationAllowed || view.state === "unavailable" || view.state === "document_entity" ? view.state : "display_restricted",
    ...
    data: observationAllowed ? view.data : null,
  };
}
...
function sanitizeAtlasFeaturedSnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== "object") return null;
  return {
    schema_version: "atlas_market_snapshot_v1",
    atlas_entity_id: snapshot.atlas_entity_id,
    instrument_id: snapshot.instrument_id,
    identity_scope: "exact_instrument",
    symbol: String(snapshot.symbol).slice(0, 80),
    last: Number(snapshot.last),
    change: Number.isFinite(Number(snapshot.change)) ? Number(snapshot.change) : null,
    change_percent: Number.isFinite(Number(snapshot.change_percent)) ? Number(snapshot.change_percent) : null,
    market_state: typeof snapshot.market_state === "string" ? snapshot.market_state.slice(0, 40) : "unknown",
    provider: String(snapshot.provider).slice(0, 80),
    provider_timestamp: snapshot.provider_timestamp,
    delay_class: typeof snapshot.delay_class === "string" ? snapshot.delay_class.slice(0, 40) : "unknown",
    stale: snapshot.stale === true,
  };
}

function sanitizeAtlasFeaturedRow(row) {
  const safeRow = sanitizeAtlasSearchRow(row);
  const providerMatches = String(row.snapshot?.provider || "").trim().toLowerCase()
    === String(safeRow.provider || "").trim().toLowerCase();
  const upstreamAllowed = safeRow.public_display_eligibility === "allowed"
    && safeRow.observation_display_eligibility === "allowed"
    && providerMatches;
  const observationDecision = atlasObservationDecision(row.snapshot?.provider, row.snapshot?.display_policy, { entityId: row.entity_id });
  const observationAllowed = upstreamAllowed && observationDecision.decision === "allowed";
  return {
    ...safeRow,
    observation_display_eligibility: observationAllowed ? "allowed" : safeRow.observation_display_eligibility === "allowed" ? "restricted" : safeRow.observation_display_eligibility,
    refusal_reason: observationAllowed ? safeRow.refusal_reason : safeRow.refusal_reason || observationDecision.reason,
    snapshot: observationAllowed ? sanitizeAtlasFeaturedSnapshot(row.snapshot) : null,
  };
}
...
function sanitizeAtlasUniversePayload(endpoint, payload) {
  ...
  if (endpoint === "eia_facets") {
    const facets = sanitizeAtlasProviderView(payload.facets, { entityId: payload.entity_id });
  ...
  if (endpoint === "eia_series") {
    const series = sanitizeAtlasProviderView(payload.series, { entityId: payload.entity_id });
...
export async function loadPublicAtlasUniverse({
  ...
  const requestTimeoutMs = finitePositive(timeoutMs, atlasUniverseTimeoutMs(env, endpoint));
  ...
  const fetched = target.ok ? await fetchJsonDocument({ env, key: `atlas_${endpoint}`, contract, fetchImpl, timeoutMs: requestTimeoutMs, target }) : target;
