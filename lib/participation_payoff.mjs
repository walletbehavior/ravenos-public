...
function summaryClause(row) {
  if (row.state === "rewarding") return `${row.subject} lead`;
  if (row.state === "fragile") return `${row.subject} are split`;
  return `${row.subject} are punishing`;
}
...
export function buildParticipationPayoffProjection(outcomesData = {}, behaviorData = {}) {
  ...
  return {
    ...
    summary: `${insights.map(summaryClause).join(". ")}.`,
