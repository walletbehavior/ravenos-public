...
function explicitOutcomeState(outcome = {}) {
  const validation = text(outcome.validation_status).toLowerCase();
  const participantOutcome = text(outcome.participant_outcome).toLowerCase();
  const direction = text(outcome.direction).toLowerCase();
  const explicit = Boolean(validation || participantOutcome || direction);
  if (!explicit) return undefined;
  const rewarding = ["favorable", "rewarding"].includes(participantOutcome) || direction === "positive";
  const punishing = ["punishing", "unfavorable"].includes(participantOutcome) || direction === "negative";
  if (rewarding === punishing) return null;
  if (rewarding) return REWARDING;
  if (punishing) return PUNISHING;
  return null;
}

function outcomeUsableSample(outcome = {}) {
  return finite(
    outcome.clean_sample
    ?? outcome.usable_sample
    ?? outcome.sample_detail?.usable
    ?? outcome.evidence_contract?.sample?.settled
    ?? outcome.evidence_contract?.sample?.usable,
  );
}

function outcomeObservedSample(outcome = {}) {
  return finite(
    outcome.observed_sample
    ?? outcome.sample_size
    ?? outcome.sample_detail?.observed
    ?? outcome.evidence_contract?.sample?.observed,
  );
}

function joinedCandidate(row, outcomes) {
  const outcome = matchingOutcome(row, outcomes);
  if (!outcome) return null;
  const explicitState = explicitOutcomeState(outcome);
  const state = explicitState === undefined ? text(row.derived_state).toLowerCase() : explicitState;
  if (![REWARDING, PUNISHING].includes(state)) return null;
  const outcomeSample = outcomeUsableSample(outcome);
  const behaviorSample = usableSample(row);
  const usable = outcomeSample === null ? behaviorSample : Math.min(behaviorSample, outcomeSample);
  const observed = outcomeObservedSample(outcome) ?? observedSample(row);
  return {
    ...row,
    derived_state: state,
    confidence: text(outcome.confidence).toLowerCase() || row.confidence,
    usable_sample: usable,
    observed_sample: observed,
    outcome_validation_status: text(outcome.validation_status).toLowerCase() || null,
    outcome_direction: text(outcome.direction).toLowerCase() || null,
    outcome_classification: text(outcome.participant_outcome).toLowerCase() || null,
  };
}
...
function isEligible(row = {}, outcomes = new Map()) {
  ...
  const outcome = matchingOutcome(row, outcomes);
  const outcomeSample = outcomeUsableSample(outcome || {});
  const outcomeConfidence = text(outcome?.confidence).toLowerCase();
  const outcomeQuality = text(outcome?.data_quality).toLowerCase();
  return row.public_safe === true
    ...
    && (outcomeSample === null || outcomeSample >= MIN_USABLE_SAMPLE)
    && (!outcomeConfidence || ["medium", "high"].includes(outcomeConfidence))
    && (!outcomeQuality || outcomeQuality === "clean");
}
...
function insight(row, requestedState, outcomes) {
  ...
  return {
    ...
    validation_status: text(outcome?.validation_status).toLowerCase() || null,
    classification_basis: explicitOutcomeState(outcome) === undefined
      ? "qualified_behavior_outcome_join"
      : "settled_forward_outcome",
  };
}
...
function joinInsightSubjects(rows) {
  const labels = rows.map((row) => text(row.subject)).filter(Boolean);
  if (labels.length < 2) return labels[0] || "";
  if (labels.length === 2) return `${labels[0]} and ${labels[1]}`;
  return `${labels.slice(0, -1).join(", ")}, and ${labels.at(-1)}`;
}

function summaryForInsights(insights) {
  const rewarding = insights.filter((row) => row.state === "rewarding");
  const fragile = insights.filter((row) => row.state === "fragile");
  const punishing = insights.filter((row) => row.state === "punishing");
  const clauses = [
    rewarding.length ? `Leadership: ${joinInsightSubjects(rewarding)}` : "",
    fragile.length ? `Mixed follow-through: ${joinInsightSubjects(fragile)}` : "",
    punishing.length ? `Weakest follow-through: ${joinInsightSubjects(punishing)}` : "",
  ].filter(Boolean);
...
export function buildParticipationPayoffProjection(outcomesData = {}, behaviorData = {}) {
  ...
  const joinedRows = rows.map((row) => joinedCandidate(row, outcomes)).filter(Boolean);
  const rewarding = selectDistinct(
    joinedRows.filter((row) => isEligible(row, outcomes) && text(row.derived_state).toLowerCase() === REWARDING),
    2,
  );
  const punishing = selectDistinct(
    joinedRows.filter((row) => isEligible(row, outcomes) && text(row.derived_state).toLowerCase() === PUNISHING),
    2,
  ...
  return {
    ...
    summary: summaryForInsights(insights),
