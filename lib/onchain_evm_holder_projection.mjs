...
function exactIdentity(identity = {}) {
  ...
  const poolAddress = clean(identity.pool_address, 42).toLowerCase();
  const tokenAddress = clean(identity.token_address, 42).toLowerCase();
  const quoteAddress = clean(identity.quote_token_address, 42).toLowerCase();
  if (!config) fail("holder_chain_unsupported", 400);
  if (!EVM_ADDRESS_RE.test(poolAddress) || !EVM_ADDRESS_RE.test(tokenAddress)) fail("holder_identity_invalid", 400);
...
function tokenFacts(payload, identity) {
  const address = clean(payload?.address_hash || payload?.address, 42).toLowerCase();
...
function holderAddress(row) {
  return clean(row?.address?.hash || row?.address_hash?.hash || row?.address_hash || row?.address, 42).toLowerCase();
}
...
async function buildHolderSnapshot({ runtime, identity, fetchImpl, observedAt }) {
  ...
  return Object.freeze({
    ...
    summary: Object.freeze({
      ...
      largest_non_pool_wallet_supply_pct: concentrationPct(internalRows, 1, facts.supply, { excludePool: true }),
      top_3_wallet_supply_pct: concentrationPct(internalRows, 3, facts.supply, { excludePool: true }),
      top_10_wallet_supply_pct: concentrationPct(internalRows, 10, facts.supply, { excludePool: true }),
    }),
