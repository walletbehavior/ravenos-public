...
Migration `0020_source_wallet_backfill_priority.sql` keeps that one-job invariant while making a large research universe responsive. History work is ordered by public demand class: customer watch, saved research, interactive lookup, verified Nexus research, then general indexed research. Within Nexus research, the transparent cohort evidence score breaks ties. If a previously discovered wallet is later searched, saved, or watched, its existing job is upgraded in place: the cursor, page evidence, lease, and provider retry cooldown are preserved. The queue retains only the shared demand class and score—not the user, watch, policy, follower count, or balance that caused the upgrade. Aggregate health reports backlog age and volume by lane so capacity can be raised from evidence rather than server size alone; it never exposes the underlying addresses or subscribers.
