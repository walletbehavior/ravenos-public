...
test("entitled user creates, pauses, and inspects an exact Raven Monitor without executable state", async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole("heading", { name: "Save and monitor this exact market" })).toBeVisible();
  await expect(page.getByText("Not monitored")).toBeVisible();
  await page.getByText("Choose qualified changes").click();
  await page.getByRole("button", { name: "Enable Raven Monitor" }).click();
  await expect(page.getByText("Raven Monitor · active")).toBeVisible();
  ...
  await expect(page.getByText("Raven Monitor · paused")).toBeVisible();
...
test("mobile Raven Monitor remains readable and delete-all removes only alert research state", async ({ page, baseURL }) => {
  ...
  await page.getByRole("button", { name: "Delete all alert research state" }).click();
  await expect(page.locator("#monitorAlertDeleteDialog")).toBeVisible();
  await page.getByRole("button", { name: "Delete monitor state" }).click();
  await expect(page.getByText("Not monitored")).toBeVisible();
...
test("unentitled and dormant states never render fake locked evidence", async ({ page, baseURL }) => {
  ...
  await page.goto("/monitor/");
  await expect(page.getByText("Raven Monitor unavailable")).toBeVisible();
  await expect(page.getByText("Raven Monitor has not been operator-granted to this account.")).toBeVisible();
  await expect(page.locator(".monitor-notification")).toHaveCount(0);
  await expect(page.getByRole("button", { name: "Enable Raven Monitor" })).toHaveCount(0);
});
