...
for (const [name, page, questionPattern] of funnelPages) {
  ...
  assert.match(page, /ravenos-evidence\.css/, `${name} should load shared evidence styles`);
  assert.match(page, /data-evidence-contract-header/, `${name} should expose the compact evidence contract header`);
  assert.match(page, /Evidence Role/, `${name} should label evidence role`);
  assert.match(page, /Usable Sample/, `${name} should show usable sample`);
  assert.match(page, /Settlement window/, `${name} should include settlement details`);
...
assert.match(liveScript, /replayOutcomeKind/);
assert.match(liveScript, /behaviorMix/);
assert.match(liveScript, /replayOutcomeRows/);
assert.match(liveScript, /memoryRareRows/);
assert.match(liveScript, /memoryTransitionRows/);
assert.match(liveScript, /behaviorMixBars/);
...
assert.match(home, /Observed Followthrough/);
assert.match(home, /id="homeCohortSample"/);
assert.match(home, /id="homeCohortMedian"/);
assert.match(home, /id="homeCohortP75"/);
assert.match(home, /Private participant identifiers are not published/);
assert.match(home, /What Happened Last Time/);
assert.match(home, /Where Opportunity Is Now/);
assert.match(home, /What Raven Is/);
assert.match(home, /Trust Layer/);
assert.match(home, /id="homeSource"/);
assert.match(home, /Live market feeds/);
assert.doesNotMatch(home, /Live public API/);
assert.match(home, /What Raven believes:/);
assert.match(home, /id="homeOpportunityTiles"/);
...
assert.match(terminal, /id="marketFeedState"/);
assert.match(terminal, /id="chartProviderState"/);
assert.match(terminal, /Wallet not connected/);
assert.match(terminal, /Instrument Coverage/);
...
assert.match(terminal, /mergeRowsByAsset/);
assert.match(terminal, /refreshMarketPrices/);
...
assert.match(terminal, /External market chart/);
assert.match(terminal, /externalChartUrl/);
assert.match(terminal, /finance\.yahoo\.com\/chart/);
assert.match(terminal, /Provider-backed Raven candles are not active/);
assert.match(terminal, /High Caps/);
assert.match(terminal, /Mega Caps/);
assert.doesNotMatch(`${home}\n${terminal}\n${opportunity}\n${solanaChain}\n${baseChain}\n${ethereumChain}`, /\bNano\b|nano caps|nano-cap|Nano survival|Nano structures/i);
assert.match(terminal, /\/api\/hyperliquid\/candles/);
assert.match(terminal, /fetchLiveCandles/);
assert.match(terminal, /candlesForRow/);
assert.match(terminal, /source=\$\{candleResult\.source/);
assert.match(terminal, /window\.setInterval\(\(\) => \{/);
assert.match(terminal, /30000/);
...
assert.match(terminal, /\["equities", "Top Stocks", "developing"\]/);
assert.match(terminal, /\["etfs", "Core ETFs", "developing"\]/);
assert.match(terminal, /\["forex", "Forex", "held"\]/);
assert.match(terminal, /\["rates", "Rates", "held"\]/);
assert.match(terminal, /\["futures", "Futures", "held"\]/);
assert.match(terminal, /EUR\/USD/);
assert.match(terminal, /US10Y/);
...
assert.match(research, /Live public research validation/);
...
assert.match(research, /Research Details/);
assert.match(research, /Latest Completed Research Cohort/);
assert.match(research, /Research Sample Forming/);
assert.doesNotMatch(research, /Developer Mode/);
...
assert.match(research, /post-observation expansion/);
...
assert.match(perps, /Perps Outcomes/);
assert.match(perps, /Forward Observations/);
assert.match(perps, /id="perpsOutcomeRows"/);
assert.match(perps, /id="perpsStrongestGroups"/);
...
assert.match(replayPage, /id="replayOutcomeRows"/);
assert.match(replayPage, /id="replaySuccessConditions"/);
assert.match(replayPage, /id="replayFailureConditions"/);
...
assert.match(outcomes, /id="out7dStatus"/);
assert.match(outcomes, /id="out30dStatus"/);
assert.match(outcomes, /id="out90dStatus"/);
assert.match(outcomes, /id="outImprovedList"/);
assert.match(outcomes, /id="outWeakenedList"/);
assert.match(outcomes, /id="outMixedList"/);
assert.match(outcomes, /Public Outcome Scorecard/);
assert.match(outcomes, /Participant Cohort Validation/);
assert.match(outcomes, /Recent Raven Reads/);
assert.match(outcomes, /Claim-to-outcome proof rail/);
assert.match(outcomes, /id="outRecentReads"/);
assert.match(outcomes, /post-observation expansion evidence/);
assert.match(outcomes, /id="outCohortSample"/);
assert.match(outcomes, /id="outCohortMedian"/);
assert.match(outcomes, /id="outCohortP75"/);
assert.match(outcomes, /id="outCohortRepeat"/);
assert.match(outcomes, /Private IDs/);
...
assert.match(memory, /id="memoryRareRows"/);
assert.match(memory, /id="memoryTransitionRows"/);
assert.match(memory, /id="memoryRarity"/);
...
assert.match(behavior, /id="behaviorMixBars"/);
assert.match(behavior, /id="behaviorBreadthScore"/);
assert.match(behavior, /id="behaviorConcentrationScore"/);
assert.match(behavior, /id="behaviorSupportList"/);
assert.match(behavior, /id="behaviorRiskList"/);
