...
test("Terminal uses PriceWorkspace by default and exposes no unresolved build token", () => {
  assert.match(terminal, /ravenos-price-workspace\.js\?v=[^_][^"']+/);
  assert.match(terminal, /RavenOSPriceWorkspace\?\.create/);
  assert.doesNotMatch(terminal, /Local structure fallback|feature_flag_off|RAVENOS_LIGHTWEIGHT_CHART_SPIKE/);
  assert.doesNotMatch(terminal, /v=__RAVENOS_BUILD_ID__/);
  assert.match(terminal, /data-ravenos-build-id>pending/);
  assert.match(terminal, /indicatorMenuToggle/);
  assert.match(terminal, /chart-panel\.indicators-open/);
});
