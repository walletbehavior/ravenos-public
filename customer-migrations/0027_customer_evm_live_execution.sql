...
-- Extend the existing noncustodial execution ledger for wallet-submitted EVM
-- swaps. Provider calldata is returned only to the authenticated browser; the
-- durable record keeps hashes and exact economic quantities, never calldata,
-- signatures, private keys, or wallet secrets.
...
ALTER TABLE ravenos_customer_live_execution_events
  RENAME TO ravenos_customer_live_execution_events_v1;
ALTER TABLE ravenos_customer_live_execution_intents
  RENAME TO ravenos_customer_live_execution_intents_v1;
...
CREATE TABLE ravenos_customer_live_execution_intents (
  ...
  fee_collection_method TEXT NOT NULL DEFAULT 'none' CHECK (
    fee_collection_method IN ('none', 'hyperliquid_builder_code', 'zero_x_integrator_fee')
  ),
...
INSERT INTO ravenos_customer_live_execution_intents (
  ...
  exact_market_id, side, order_type, notional_usdc, raven_fee_bps,
  expected_raven_fee_usdc, observed_raven_fee_usdc, fee_token, fee_recipient,
  fee_collection_method, fee_collection_status, state, prepared_payload_hash,
  provider_request_id, prepared_json, expires_at, created_at, updated_at
)
SELECT
  execution_id, schema_version, user_id, venue, chain_namespace, wallet_address,
  exact_market_id, side, order_type, notional_usdc, raven_fee_bps,
  ...
  fee_collection_method, fee_collection_status, state, prepared_payload_hash,
  provider_request_id, prepared_json, expires_at, created_at, updated_at
FROM ravenos_customer_live_execution_intents_v1;
...
SELECT event_id, execution_id, state, evidence_json, observed_at
FROM ravenos_customer_live_execution_events_v1;

DROP TABLE ravenos_customer_live_execution_events_v1;
DROP TABLE ravenos_customer_live_execution_intents_v1;
