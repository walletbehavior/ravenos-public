...
assert.equal(config.privy_wallets.implementation_status, "owner_evm_wallet_canary_release_ready");
...
assert.equal(config.privy_wallets.explicit_user_allowlist_required, true);
assert.equal(config.privy_wallets.wildcard_allowlist_for_initial_canary, false);
...
assert.equal(config.privy_wallets.identity_jwks_rotation_bounded_by_kid, true);
assert.equal(config.privy_wallets.public_jwks_bootstrap_release_enabled, true);
assert.equal(config.privy_wallets.dashboard_jwt_configuration_saved, true);
...
assert.equal(config.privy_wallets.solana_activation_blocked_on_jupiter_fee_configuration, true);
...
assert.equal(config.wallet_copy.server_owned_pro_entitlement_required, false);
assert.equal(config.wallet_copy.basic_wallet_lookup_included, true);
assert.equal(config.wallet_copy.basic_wallet_screener_included, true);
assert.equal(config.wallet_copy.raven_copy_subscription_required, false);
assert.equal(config.wallet_copy.pro_required_for_advanced_cohorts_behavior_profit_quality_deep_history_and_copyability, true);
assert.deepEqual(config.wallet_copy.supported_chains, ["solana", "robinhood", "bsc", "base", "ethereum"]);
assert.deepEqual(config.wallet_copy.persistently_indexed_chains, ["solana", "robinhood"]);
assert.deepEqual(config.wallet_copy.bounded_on_demand_evm_lookup_chains, ["robinhood", "bsc", "base", "ethereum"]);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_provider, "blockscout_pro_v2");
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_release_enabled, true);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_cache_api_seconds, 60);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_isolate_local_cache, false);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_trade_classification, false);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_pnl_claimed, false);
assert.equal(config.wallet_copy.bounded_on_demand_evm_lookup_copy_eligible, false);
...
assert.equal(config.wallet_copy.shadow_copy_release_enabled, true);
...
assert.equal(config.wallet_copy.deep_history_backfill_active, true);
...
assert.equal(config.wallet_copy.deep_history_priority_lanes_active, true);
...
assert.equal(config.wallet_copy.source_sell_mapping_active, true);
...
assert.equal(config.wallet_copy.shared_prospective_copyability_screener_projection_active, true);
...
assert.equal(config.wallet_copy.shared_prospective_follower_outcome_checkpoints_active, true);
...
assert.equal(config.wallet_copy.production_activation_completed, true);
assert.equal(config.wallet_copy.continuous_observer_active, true);
assert.equal(config.wallet_copy.continuous_observer_transport, "bounded_rpc_poll");
assert.equal(config.wallet_copy.continuous_observer_interval_seconds, 300);
assert.equal(config.wallet_copy.continuous_observer_maximum_wallets_per_run, 50);
assert.equal(config.wallet_copy.continuous_observer_only_polls_baselined_active_watches, true);
assert.equal(config.wallet_copy.continuous_observer_nexus_latency_claimed, false);
assert.equal(config.wallet_copy.scheduler_active, true);
assert.equal(config.wallet_copy.all_activation_controls_default_off, false);
...
const solanaFeeMigration = readFileSync(join(root, "customer-migrations/0031_solana_jupiter_referral_fees.sql"), "utf8");
...
assert.match(solanaLiveExecution, /solana_live_fee_policy_required/);
assert.match(solanaLiveExecution, /settled_referral_fee_credit_mismatch/);
assert.match(solanaLiveExecution, /collection_method:\s*"jupiter_referral_program"/);
...
assert.match(solanaFeeMigration, /jupiter_referral_program/);
assert.match(solanaFeeMigration, /ravenos_customer_live_execution_events_append_only/);
assert.doesNotMatch(solanaFeeMigration, /private_key|seed_phrase|signature\s+TEXT/i);
